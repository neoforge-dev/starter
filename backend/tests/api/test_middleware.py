...
def valid_jwt_token(test_settings: Settings) -> str:
    ...
    payload = {"sub": "test-client", "exp": datetime.now(UTC) + timedelta(minutes=5)}
    return jwt.encode(
        payload,
...
async def test_error_handler_middleware_validation_error(
    ...
    response = await client.post(
        f"{test_settings.api_v1_str}/items/", json=invalid_item_data, headers=headers
    )
...
async def test_rate_limit_middleware_no_redis_connection(
    app_with_middleware: FastAPI, client: AsyncClient, test_settings: Settings
):
