...
from app.models.tenant import Organization, OrganizationMembership, Tenant, TenantStatus
...
from tests.factories import OrganizationFactory, TenantFactory, UserFactory
...
class TestTenantIsolation:
    ...
    async def test_tenant_data_isolation(
        ...
        response = await client.get(
            ...
            headers={**superuser_token_headers, "X-Tenant-ID": str(test_tenant.uuid)},
        )
        ...
        response = await client.get(
            ...
            headers={**superuser_token_headers, "X-Tenant-ID": str(other_tenant.uuid)},
        )
        assert response.status_code == 404  # Should not find org from other tenant
    ...
    async def test_tenant_middleware_resolution(
        ...
        response = await client.get(
            "/api/v1/health", headers={"X-Tenant-ID": str(test_tenant.uuid)}
        )
    ...
    async def test_tenant_status_enforcement(
        ...
        cancelled_tenant = await TenantFactory.create(db, status=TenantStatus.CANCELLED)
    ...
    async def test_cross_tenant_data_leakage_prevention(
        ...
        for i in range(3):
            org1 = await OrganizationFactory.create(
                db, tenant_id=test_tenant.id, slug=f"tenant1-org-{i}"
            )
            tenant1_orgs.append(org1)

            org2 = await OrganizationFactory.create(
                db, tenant_id=other_tenant.id, slug=f"tenant2-org-{i}"
            )
    ...
    async def test_tenant_schema_isolation(
        self, db: AsyncSession, test_tenant: Tenant, other_tenant: Tenant
    ):
...
class TestRBACIsolation:
    ...
    async def test_permission_enforcement(
        ...
        org_data = {"slug": "test-org", "name": "Test Organization", "type": "team"}

        response = await client.post(
            "/api/v1/organizations/",
            headers={**normal_user_token_headers, "X-Tenant-ID": str(test_tenant.uuid)},
            json=org_data,
        )
    ...
    async def test_organization_membership_isolation(
        ...
        membership1 = OrganizationMembership(
            user_id=user1.id, organization_id=org1.id, status="active"
        )
        db.add(membership1)

        membership2 = OrganizationMembership(
            user_id=user2.id, organization_id=org2.id, status="active"
        )
        db.add(membership2)
...
class TestSecurityValidation:
    ...
    async def test_tenant_enumeration_prevention(
        ...
        response = await client.get(
            "/api/v1/organizations/", headers={"X-Tenant-ID": "invalid-uuid"}
        )
        ...
        response = await client.get(
            "/api/v1/organizations/", headers={"X-Tenant-ID": fake_uuid}
        )
    ...
    async def test_sql_injection_prevention(
        self, client: AsyncClient, test_tenant: Tenant, superuser_token_headers: dict
    ):
...
class OrganizationFactory:
    ...
    @staticmethod
    async def create(
        db: AsyncSession, tenant_id: int, slug: str = None, name: str = None, **kwargs
    ) -> Organization:
