...
class TestRoleAssignment:
    ...
    async def test_assign_role_to_user(
        ...
        assignment_data = {"user_id": normal_user.id, "tenant_id": test_tenant.id}
...
class TestPermissionAPI:
    ...
    async def test_list_permissions(
        ...
        response = await client.get(
            "/api/v1/rbac/permissions", headers=superuser_token_headers
        )
    ...
    async def test_search_permissions(
        ...
        # Search by keyword
        response = await client.get(
            "/api/v1/rbac/permissions?search=special", headers=superuser_token_headers
        )
...
@pytest.fixture
async def test_role(db: AsyncSession, test_tenant: Tenant) -> Role:
    """Create a test role."""
    role = Role(
        name="test_role",
        display_name="Test Role",
