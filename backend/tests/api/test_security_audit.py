...
class TestSecurityAuditor:
    ...
    async def test_security_dashboard_data(self, db_session: AsyncSession):
        ...
        # Get dashboard data
        dashboard_data = await security_auditor.get_security_dashboard_data(db_session, hours=24)
    ...
    async def test_user_security_history(self, db_session: AsyncSession):
        ...
        history = await security_auditor.get_user_security_history(db_session, user_id, limit=5)
        ...
        if len(history) > 1:
            timestamps = [datetime.fromisoformat(event["timestamp"].replace("Z", "+00:00")) for event in history]
...
class TestSecurityAPI:
    ...
    async def test_security_dashboard_endpoint(
        ...
        response = await async_client.get("/api/v1/security/dashboard?hours=24", headers=headers)
