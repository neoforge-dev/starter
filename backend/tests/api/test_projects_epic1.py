...
async def test_projects_crud_and_pagination(client: AsyncClient):
    ...
    r1 = await client.post(
        "/api/v1/projects", json={"name": "A", "description": "x"}, headers=headers
    )
    ...
    rbeyond = await client.get(
        "/api/v1/projects", params={"page": pages + 10, "page_size": 50}
    )
    ...
    gid2 = await client.get(
        f"/api/v1/projects/{p1['id']}", headers={"If-None-Match": etag}
    )
    assert gid2.status_code == 304

    # Patch idempotency
    patch_headers = {"Idempotency-Key": "key-1-patch"}
    up1 = await client.patch(
