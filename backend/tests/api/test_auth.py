...
async def test_login_access_token(
    client: AsyncClient, db: AsyncSession, test_user: User, test_settings: Settings
) -> None:
    ...
    logger.info(
        f"Attempting login for user: {test_user.email}, PW snippet: {test_user.plain_password[:3]}..."
    )
    existing_user = await user_crud.get_by_email(db, email=test_user.email)
    if existing_user:
        logger.info(
            f"User {test_user.email} found in DB (ID: {existing_user.id}) before client call."
        )
...
async def test_read_users_me(
    ...
    response = await client.get(
        f"{test_settings.api_v1_str}/users/me", headers=test_user_headers
    )  # Use test_settings, corrected endpoint path
...
async def test_read_users_me_invalid_token(
    ...
    response = await client.get(
        f"{test_settings.api_v1_str}/users/me", headers=headers
    )  # Use test_settings, corrected endpoint path
    assert response.status_code == 401
    data = response.json()
    assert "detail" in data
...
async def test_register_user_email_integration(
    client: AsyncClient, db: AsyncSession, test_settings: Settings
) -> None:
...
async def test_register_user_email_failure_continues_registration(
    client: AsyncClient, db: AsyncSession, test_settings: Settings
) -> None:
    ...
    with patch(
        "app.api.v1.endpoints.auth.send_new_account_email",
