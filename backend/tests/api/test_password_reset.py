...
@pytest.fixture
async def reset_test_user(db: AsyncSession, test_user_email: str, test_user_password: str) -> User:
...
class TestPasswordResetRequest:
    ...
    async def test_password_reset_request_success(
        ...
        with patch('app.api.v1.endpoints.auth.send_reset_password_email') as mock_send_email:
            ...
            data = response.json()
            assert data["message"] == "If the email address is registered, you will receive a password reset link shortly."
        ...
        result = await db.execute(
            select(PasswordResetToken).where(PasswordResetToken.user_id == reset_test_user.id)
        )
    ...
    async def test_password_reset_request_email_failure_continues(
        ...
        with patch('app.api.v1.endpoints.auth.send_reset_password_email', side_effect=Exception("SMTP error")):
            response = await client.post(
                f"{test_settings.api_v1_str}/auth/reset-password-request",
                json=request_data
            )
...
class TestPasswordResetConfirm:
    ...
    async def test_password_reset_confirm_success(
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["message"] == "Password has been successfully reset. You can now log in with your new password."
