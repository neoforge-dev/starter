...
async def test_verify_email_success(
    ...
    verification_token = create_access_token(
        subject=user.id, settings=test_settings, expires_delta=timedelta(hours=24)
    )
...
async def test_verify_email_nonexistent_user_token(
    client: AsyncClient, db: AsyncSession, test_settings: Settings
) -> None:
...
async def test_resend_verification_success(
    ...
    with patch("app.api.v1.endpoints.auth.send_new_account_email") as mock_send_email:
        ...
        response = await client.post(
            f"{test_settings.api_v1_str}/auth/resend-verification", json=resend_data
        )
...
async def test_resend_verification_nonexistent_email(
    ...
    data = response.json()
    assert (
        "verification link shortly" in data["message"]
...
async def test_user_registration_creates_unverified_user(
    client: AsyncClient, db: AsyncSession, test_settings: Settings
) -> None:
...
async def test_verify_email_integration_flow(
    client: AsyncClient, db: AsyncSession, test_settings: Settings
) -> None:
...
async def test_resend_verification_rate_limiting(
    ...
    assert (
        "verification link shortly" in data["message"]
    )  # Same generic message even when rate limited
