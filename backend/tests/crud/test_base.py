...
async def test_create(db: AsyncSession, crud: TestCRUD):
    ...
    item_in = TestItemCreate(
        title="Test Item", description="Test Description", owner_id=1
    )
...
async def test_get_multi_by_owner(db: AsyncSession, crud: TestCRUD):
    ...
    for owner_id in owner_ids:
        item_in = TestItemCreate(
            title=f"Test Item for owner {owner_id}", owner_id=owner_id
        )
    ...
    assert (
        len(items_owner1) == 2
    ), f"Expected 2 items for owner 1, got {len(items_owner1)}"
    ...
    assert (
        len(items_owner2) == 3
    ), f"Expected 3 items for owner 2, got {len(items_owner2)}"
    ...
    assert (
        len(items_owner999) == 1
    ), f"Expected 1 item for owner 999, got {len(items_owner999)}"
    assert items_owner999[0].title == "Initial Item"

    # Test pagination within get_multi_by_owner
    items_owner2_paginated = await crud.get_multi_by_owner(
        db, owner_id=2, skip=1, limit=1
    )
    assert (
        len(items_owner2_paginated) == 1
    ), f"Expected 1 paginated item for owner 2, got {len(items_owner2_paginated)}"
...
async def test_update(db: AsyncSession, crud: TestCRUD):
    ...
    # Create test item
    item_in = TestItemCreate(
        title="Original Title", description="Original Description", owner_id=1
    )
    item = await crud.create(db, obj_in=item_in)
    ...
    updated_item = await crud.update(db, db_obj=item, obj_in=item_update)
