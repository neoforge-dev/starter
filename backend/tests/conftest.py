...
from app.db.query_monitor import after_cursor_execute, before_cursor_execute
...
async def db(engine) -> AsyncGenerator[AsyncSession, None]:
    ...
    async with engine.connect() as connection:
        ...
        event.listen(
            connection.sync_connection, "before_cursor_execute", before_cursor_execute
        )
        event.listen(
            connection.sync_connection, "after_cursor_execute", after_cursor_execute
        )
        ...
        try:
            ...
            logger.warning(
                f"Could not detach event listeners: {e}"
            )  # Log warning if removal fails
    ...
    try:
        ...
        if "redis" in locals() and redis:
            try:
                ...
                logger.error(
                    f"Error closing Redis connection: {close_exc}", exc_info=True
                )
...
async def client(db: AsyncSession, test_settings: Settings) -> AsyncClient:
    ...
    from app.db.session import (
        get_db as app_session_get_db,  # Import the one used by security
    )
    ...
    logger.debug(
        f"Overriding get_settings for main app. Effective settings: {test_settings}"
    )
...
async def admin_user(db: AsyncSession) -> Admin:
    ...
    logger.info(
        f"Created underlying user {user_obj.email} with ID {user_obj.id} for admin"
    )
    ...
    admin_obj = await admin.create(
        db=db, obj_in=admin_in, actor_id=user_obj.id, user_id=user_obj.id
    )
    await db.flush()
...
async def readonly_admin_user(db: AsyncSession) -> Admin:
    ...
    admin_obj = await admin_crud.create(
        db=db, obj_in=admin_in, actor_id=user.id, user_id=user.id
    )
    await db.flush()
    await db.refresh(admin_obj)
    logger.info(
        f"Created read-only admin user {user.email} with ID {admin_obj.id}, User ID {admin_obj.user_id}"
    )
...
async def admin_user_token_headers(
    ...
    logger.info(
        f"Created admin user {admin_user_obj.user.email} with Admin ID {admin_user_obj.id} for token generation"
    )
    ...
    logger.info(
        f"Generated token headers for admin user ID {user_id_for_token} (Admin ID {admin_user_obj.id})"
    )
