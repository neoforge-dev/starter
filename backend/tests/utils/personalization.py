...
async def create_random_personalization_profile(
    db: AsyncSession, *, user_id: int, segment: Optional[str] = None, **kwargs
) -> PersonalizationProfile:
    ...
    profile_data = {
        ...
        "secondary_segments": random.choice(
            [None, ["mobile_first"], ["desktop_focused"], ["social_user"]]
        ),
        ...
        "notification_preferences": {
            ...
            "channels": random.sample(
                ["email", "in_app", "push"], k=random.randint(1, 3)
            ),
    ...
    rule_data = PersonalizationRuleCreate(
        ...
        conditions={
            ...
            "device_types": random.choice(
                [None, ["mobile"], ["desktop"], ["mobile", "tablet"]]
            ),
        ...
        starts_at=random.choice(
            [None, datetime.utcnow() - timedelta(days=random.randint(1, 30))]
        ),
        expires_at=random.choice(
            [None, datetime.utcnow() + timedelta(days=random.randint(30, 90))]
        ),
        **kwargs,
    )
...
async def create_random_personalization_interaction(
    ...
    **kwargs,
    ...
    return await personalization_interaction.create_interaction(
        db, obj_in=interaction_data
    )


def create_sample_personalization_request(
    user_id: int, context: str = "dashboard", **kwargs
) -> Dict[str, Any]:
...
def create_sample_rule_conditions() -> Dict[str, Any]:
    ...
    return {
        ...
        "device_types": random.choice(
            [None, ["mobile"], ["desktop"], ["mobile", "tablet"], ["desktop", "mobile"]]
        ),
        "time_of_day": random.choice(
            [None, {"start": 9, "end": 17}, {"start": 18, "end": 23}]
        ),
        "day_of_week": random.choice(
            [None, [0, 1, 2, 3, 4], [5, 6]]  # Weekdays  # Weekends
        ),
        "feature_usage": random.choice(
            [None, {"analytics": 5, "reports": 3}, {"dashboard": 10}]
        ),
        "ui_preferences": random.choice(
            [None, {"theme": "dark"}, {"layout_density": "compact"}]
        ),
    }
...
async def create_personalization_test_scenario(
    ...
    *,
    ...
    scenario_data = {"users": [], "profiles": [], "rules": [], "interactions": []}
...
def assert_rule_applies_to_user(
    rule: Dict[str, Any], profile: Dict[str, Any], context: str
) -> None:
    ...
    assert any(
        segment in rule["target_segments"] for segment in user_segments
...
def generate_realistic_user_journey(
    user_id: int, rule_ids: List[str], session_duration_minutes: int = 30
) -> List[Dict[str, Any]]:
