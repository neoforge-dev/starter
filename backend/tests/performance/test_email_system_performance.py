...
class PerformanceMetrics:
    ...
    def percentile_95(self) -> float:
        ...
        if self.operation_times:
            return statistics.quantiles(self.operation_times, n=20)[
                18
            ]  # 95th percentile
        return 0.0
...
class TestEmailQueuePerformance:
    ...
    async def test_concurrent_queue_access(self) -> None:
        ...
        async def consumer_task(consumer_id: int):
            processed = 0
            while (
                processed < (num_producers * emails_per_producer) // num_consumers + 5
            ):
...
class TestEmailWorkerPerformance:
    ...
    async def test_worker_under_error_conditions(self) -> None:
        ...
        assert (
            metrics.operations_per_second > 30
        )  # At least 30 ops/sec even with errors
...
class TestWebhookPerformance:
    ...
    async def test_high_volume_webhook_processing(
        ...
        try:
            ...
            for batch_start in range(0, num_tracking_records, events_per_batch):
                ...
                for i in range(
                    batch_start,
                    min(batch_start + events_per_batch, num_tracking_records),
                ):
                    event = {
            ...
            print(
                f"  Events/sec: {metrics.operations_per_second * events_per_batch:.1f}"
            )
    ...
    async def test_concurrent_webhook_requests(
        ...
        try:
            ...
            assert metrics.success_rate == 100.0
            assert (
                metrics.operations_per_second > 20
            )  # At least 20 concurrent requests/sec
...
class TestEndToEndPerformance:
        ...
        try:
            ...
            async def registration_batch(batch_id: int):
                    ...
                    for i, response in enumerate(responses):
                        ...
                        if success:
                            user = await user_crud.get_by_email(
                                db, email=f"load-test-{user_id}@example.com"
                            )
                            if user:
                ...
                metrics.record_operation(
                    batch_end - batch_start, len(batch_users) == users_per_batch
                )
                return batch_users
    ...
    async def test_system_resource_usage_monitoring(
        ...
        if len(metrics.memory_usage) > 1:
            ...
            )  # Memory should remain stable (< 100MB variance)

        assert (
            metrics.operations_per_second >= operations_per_second * 0.8
        )  # Within 80% of target
        ...
        if metrics.memory_usage:
            print(
                f"  Memory stability: {max(metrics.memory_usage) - min(metrics.memory_usage):.1f}MB variance"
            )
