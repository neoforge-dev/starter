...
from tests.factories import CommunityPostFactory, ProjectFactory, SupportTicketFactory
...
class TestCursorPaginationSecurity:
    ...
    def test_cursor_encoding_decoding(self):
        ...
        assert (
            len(cursor) > 50
        )  # Should be substantial length due to base64 + signature
    ...
    def test_cursor_with_datetime_values(self):
        ...
        cursor = manager.encode_cursor(sort_by="created_at", last_value=dt)
...
class TestCursorPaginationQueries:
    ...
    async def test_cursor_query_with_pagination(self, db_session: AsyncSession):
        ...
        # Create test data
        projects = [ProjectFactory(name=f"Project {i}") for i in range(5)]
        ...
        result = await manager.paginate(
            db=db_session, base_query=base_query, model=Project, limit=2
        )
...
class TestBackwardCompatibility:
    ...
    async def test_offset_pagination_still_works(
...
class TestPerformanceValidation:
    ...
    async def test_response_time_under_200ms(
        ...
        assert (
            duration_ms < 200
        ), f"Response took {duration_ms:.2f}ms, exceeding 200ms SLA"
        ...
        assert (
            duration_ms < 200
        ), f"Filtered response took {duration_ms:.2f}ms, exceeding 200ms SLA"
        ...
        for position, cursor in positions_to_test:
            if position == "middle":
                ...
                mid_response = client.get(
                    "/api/v1/projects?limit=1000&sort_by=created_at"
                )
                ...
                for _ in range(8):  # Navigate through pages
                    resp = client.get(
                        f"/api/v1/projects?limit=1000&sort_by=created_at{'&cursor=' + current_cursor if current_cursor else ''}"
                    )
            ...
            assert (
                duration_ms < 200
            ), f"Response at {position} position took {duration_ms:.2f}ms"
...
class TestErrorHandling:
    ...
    async def test_empty_dataset_pagination(
        self, client: TestClient, db_session: AsyncSession
    ):
...
class TestMetricsCollection:
    ...
    async def test_metrics_collection_during_pagination(
        self, client: TestClient, db_session: AsyncSession
    ):
...
class TestCacheIntegration:
    ...
    async def test_etag_generation_for_pagination(
        self, client: TestClient, db_session: AsyncSession
    ):
    ...
    async def test_conditional_requests(
        self, client: TestClient, db_session: AsyncSession
    ):
        ...
        response2 = client.get(
            "/api/v1/projects?limit=5", headers={"If-None-Match": etag}
        )
...
class TestEndpointIntegration:
    ...
    async def test_projects_endpoint_cursor_pagination(
        self, client: TestClient, db_session: AsyncSession
    ):
    ...
    async def test_support_tickets_endpoint_cursor_pagination(
        self, client: TestClient, db_session: AsyncSession
    ):
    ...
    async def test_community_posts_endpoint_cursor_pagination(
        self, client: TestClient, db_session: AsyncSession
    ):
...
class TestPerformanceBenchmarks:
    ...
    async def test_cursor_vs_offset_performance(
        self, client: TestClient, db_session: AsyncSession
    ):
