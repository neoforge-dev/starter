            ...
            for tracking in tracking_records:
                if tracking.email_id == email_id:
                    await email_tracking.update_status(
                        db=db, db_obj=tracking, status=EmailStatus.SENT
                    )
                    await db.commit()
                    break
    ...
    async def test_concurrent_auth_operations_with_email_system(
        ...
        try:
            ...
            for i, response in enumerate(responses):
                ...
                user = await user_crud.get_by_email(
                    db, email=f"concurrent-auth-{i}@example.com"
                )
                assert user is not None
                users_created.append(user)
    ...
    async def test_email_system_recovery_after_maintenance(
        ...
        try:
            with patch(
                ...
                response = await client.post(
                    f"{test_settings.api_v1_str}/auth/register", json=maintenance_data
                )
                ...
                response = await client.post(
                    f"{test_settings.api_v1_str}/auth/register", json=recovery_data
                )
                ...
                recovery_user = await user_crud.get_by_email(
                    db, email=recovery_data["email"]
                )
