...
async def test_tz_datetime_type_non_utc(db: AsyncSession):
    ...
    for tz_name in timezones:
        ...
        assert model.tz_datetime.utcoffset() == timedelta(
            0
        ), f"Expected UTC offset for {tz_name}"
        # Compare date components based on the UTC representation of the original time
        now_utc = now.astimezone(timezone.utc)
        assert (
            model.tz_datetime.year,
            model.tz_datetime.month,
            model.tz_datetime.day,
        ) == (
            now_utc.year,
            now_utc.month,
            now_utc.day,
        ), f"Date components mismatch for {tz_name}"
...
async def test_utc_datetime_type_from_utc(db: AsyncSession):
    ...
    assert (
        model.utc_datetime.year,
        model.utc_datetime.month,
        model.utc_datetime.day,
...
async def test_tz_datetime_naive_input(db: AsyncSession):
    ...
    assert model.tz_datetime.tzinfo is not None
    assert model.tz_datetime.tzinfo == timezone.utc
    assert model.tz_datetime.replace(tzinfo=None) == naive_now.replace(
        microsecond=model.tz_datetime.microsecond
    )
...
async def test_utc_datetime_naive_input(db: AsyncSession):
    ...
    assert model.utc_datetime.tzinfo == timezone.utc
    assert model.utc_datetime.replace(tzinfo=None) == naive_now.replace(
        microsecond=model.utc_datetime.microsecond
    )
...
async def test_tz_datetime_dst_handling(db: AsyncSession):
    ...
    for dt in dst_dates:
        ...
        assert model.tz_datetime.utcoffset() == timedelta(
            0
        ), f"Expected UTC offset for DST date {dt}"
        assert (
            model.tz_datetime.year,
            model.tz_datetime.month,
            model.tz_datetime.day,
...
async def test_utc_datetime_dst_handling(db: AsyncSession):
    ...
    for dt in dst_dates:
        ...
        assert (
            model.utc_datetime.tzinfo is not None
        ), f"utc_datetime tzinfo is None for {dt}"
        assert model.utc_datetime.utcoffset() == timedelta(0)
        assert (
            model.utc_datetime.year,
            model.utc_datetime.month,
