...
class TestUserRegistrationEmailFlow:
    ...
    async def test_complete_registration_flow(
        ...
        response = await client.post(
            f"{test_settings.api_v1_str}/auth/verify-email", json=verification_data
        )
    ...
    async def test_complete_password_reset_flow(
        ...
        try:
            ...
            with patch(
                ...
                side_effect=mock_send_reset_email,
            ):
                response = await client.post(
            ...
            login_data = {"username": user.email, "password": "oldpassword123"}
    ...
    async def test_password_reset_with_email_tracking(
        ...
        try:
            ...
            tracking_records = await db.execute(
                db.select(EmailTracking).where(EmailTracking.recipient == user.email)
            )
...
class TestConcurrentEmailProcessing:
    ...
    async def test_concurrent_user_registrations(
        ...
        with patch(
            "app.api.v1.endpoints.auth.send_new_account_email", new_callable=AsyncMock
        ):
        ...
        try:
            for i, response in enumerate(responses):
                ...
                user = await user_crud.get_by_email(
                    db, email=f"concurrent-{i}@example.com"
                )
                assert user is not None
