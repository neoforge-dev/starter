...
from typing import Any, Coroutine, Generic, List, Optional, Sequence, Type, TypeVar
...
from app.schemas.event import EventSource, EventType
...
class AsyncModelFactory(factory.Factory):
    ...
    @classmethod
    async def create(cls, session: AsyncSession, **kwargs: Any) -> SQLModel:
...
class ModelFactory(factory.Factory):
    ...
    @classmethod
    def _create(
        cls, model_class: Type[SQLModel], *args: Any, **kwargs: Any
    ) -> SQLModel:
        """
...
class UserFactory(AsyncModelFactory):
    ...
    async def _create(
        ...
        **kwargs: Any,
        ...
        from app.core.auth import (  # Import here to avoid circular import
            get_password_hash,
        )
        ...
        if "full_name" not in kwargs:
            kwargs["full_name"] = cls.full_name.evaluate(
...
class UserCreateFactory(ModelFactory):
    ...
    password = factory.Faker(
        "password", length=12, special_chars=True, digits=True, locale="en_US"
    )
...
class AdminFactory(ModelFactory):
    ...
    @classmethod
    async def _create(
        cls, model_class: Type[Admin], session: AsyncSession, **kwargs
    ) -> Admin:
    ...
    async def _create(
        ...
        **kwargs: Any,
        ...
        if "description" not in kwargs:
            kwargs["description"] = cls.description.evaluate(
                None, None, {"locale": "en_US"}
            )
...
class EventFactory(AsyncModelFactory):
    ...
    page_url = factory.Maybe(
        "event_type", yes_declaration=Faker("url", locale="en_US"), no_declaration=None
    )
    ...
    ip_address = factory.Maybe(
        "user_id", yes_declaration=Faker("ipv4", locale="en_US"), no_declaration=None
    )
