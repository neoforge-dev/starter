...
async def test_send_email():
    ...
    with patch(
        "app.core.email.FastMail.send_message", new_callable=AsyncMock
    ) as mock_send_message:
        # We still need EmailService to be instantiated, but we don't need to mock its internals fully
        with patch(
            "app.core.email.Redis.from_url", new_callable=MagicMock
        ):  # Mock Redis for queue init
            with patch.dict(
                os.environ, DUMMY_MAIL_ENV, clear=True
            ):  # Mock env for ConnectionConfig
                # Call the function under test (which creates EmailService and calls _send_direct_email)
                await send_email(
                    db=None,
                    email_id="test_id_456",
...
async def test_send_test_email():
    ...
    with patch(
        "app.core.email.EmailQueue.enqueue", new_callable=AsyncMock
    ) as mock_enqueue:
...
async def test_send_reset_password_email():
    ...
    assert (
        queued_item.template_data["reset_link"]
        == f"{settings.frontend_url}/reset-password?token=reset_token_123"
    )
...
async def test_send_new_account_email():
    ...
    assert queued_item.template_data["username"] == "testuser"
    assert (
        queued_item.template_data["verify_link"]
        == f"{settings.frontend_url}/verify-email?token=verify_token_123"
    )
    assert queued_item.template_data["valid_hours"] == 24
