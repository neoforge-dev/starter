...
from app.core.config import get_settings  # Keep import for cache test
from app.core.config import Environment, Settings, parse_bool_str, parse_environment
...
def test_settings_cors_validator_in_test_mode():
    ...
    ), "When testing=True, cors_origins should be empty"
...
class TestOTLPConfiguration:
    ...
    def test_otel_traces_exporter_validation(self):
        ...
        for exporter in ["console", "otlp", "none"]:
            settings = Settings(
                secret_key=VALID_SECRET_KEY, otel_traces_exporter=exporter
            )
            assert settings.otel_traces_exporter == exporter

        # Invalid value
        with pytest.raises(
            ValidationError, match="OTEL_TRACES_EXPORTER must be one of"
        ):
            Settings(secret_key=VALID_SECRET_KEY, otel_traces_exporter="invalid")
    ...
    def test_otel_traces_sampler_validation(self):
        ...
        for sampler in ["always_on", "always_off", "ratio", "traceidratio"]:
            settings = Settings(
                secret_key=VALID_SECRET_KEY, otel_traces_sampler=sampler
            )
            assert settings.otel_traces_sampler == sampler

        # Invalid value
        with pytest.raises(ValidationError, match="OTEL_TRACES_SAMPLER must be one of"):
            Settings(secret_key=VALID_SECRET_KEY, otel_traces_sampler="invalid")
    ...
    def test_otel_exporter_otlp_protocol_validation(self):
        ...
        for protocol in ["grpc", "http/protobuf", "http/json"]:
            settings = Settings(
                secret_key=VALID_SECRET_KEY, otel_exporter_otlp_protocol=protocol
            )
            assert settings.otel_exporter_otlp_protocol == protocol

        # Invalid value
        with pytest.raises(
            ValidationError, match="OTEL_EXPORTER_OTLP_PROTOCOL must be one of"
        ):
            Settings(secret_key=VALID_SECRET_KEY, otel_exporter_otlp_protocol="invalid")
    ...
    def test_otel_sdk_disabled_validation(self):
        ...
        for value, expected in [
            ...
            settings = Settings(secret_key=VALID_SECRET_KEY, otel_sdk_disabled=value)
    ...
    def test_otel_endpoint_configuration(self):
        ...
        assert (
            ...
            == "http://jaeger:4318/v1/traces/custom"
        )
