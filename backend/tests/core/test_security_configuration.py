...
class TestSecurityConfigurationValidation:
    ...
    def test_production_cors_requires_https(self):
        ...
        assert any(
            "Production CORS origins must use HTTPS" in str(error.get("msg", ""))
            for error in errors
        )
    ...
    def test_production_cors_blocks_wildcard(self):
        ...
        errors = exc_info.value.errors()
        assert any(
            "Wildcard CORS origins" in str(error.get("msg", "")) for error in errors
        )
    ...
    def test_secret_key_validation(self):
        ...
        settings = Settings(secret_key="test_secret_key_at_least_32_chars_long")
        assert (
    ...
    def test_cors_credentials_configuration(self):
        ...
        settings = Settings(
            secret_key="test_secret_key_at_least_32_chars_long", cors_credentials=True
        )

        assert settings.cors_credentials is True

        settings_false = Settings(
            secret_key="test_secret_key_at_least_32_chars_long", cors_credentials=False
        )
