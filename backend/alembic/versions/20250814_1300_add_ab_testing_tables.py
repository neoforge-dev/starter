...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('test_key', sa.String(100), nullable=False, unique=True, index=True,
        ...
        sa.Column('secondary_metrics', postgresql.JSON(), nullable=True,
        ...
        sa.Column('targeting_rules', postgresql.JSON(), nullable=True,
        ...
        sa.Column('significance_reached_at', sa.DateTime(), nullable=True,
    ...
    op.create_index('idx_ab_tests_status_dates', 'ab_tests', 
                   ['status', 'start_date', 'end_date'])
    op.create_index('idx_ab_tests_active', 'ab_tests', 
                   ['status', 'start_date', 'end_date'],
                   postgresql_where=sa.text("status = 'active'"))
    op.create_index('idx_ab_tests_creator', 'ab_tests', 
                   ['created_by', 'created_at'])
    op.create_index('idx_ab_tests_significance', 'ab_tests', 
                   ['is_statistically_significant', 'status'])
    
    # Create indexes for ab_test_variants table
    op.create_index('idx_ab_test_variants_test_key', 'ab_test_variants', 
                   ['test_id', 'variant_key'])
    op.create_index('idx_ab_test_variants_performance', 'ab_test_variants', 
                   ['test_id', 'conversion_rate'])
    op.create_unique_index('uq_ab_test_variants_test_key', 'ab_test_variants', 
                          ['test_id', 'variant_key'])
    ...
    op.create_unique_index(
        'uq_ab_test_assignments_user', 'ab_test_assignments', 
        ['test_id', 'user_id'],
        postgresql_where=sa.text('user_id IS NOT NULL')
    )
    op.create_unique_index(
        'uq_ab_test_assignments_session', 'ab_test_assignments', 
        ['test_id', 'session_id'],
        postgresql_where=sa.text('session_id IS NOT NULL')
    )
    op.create_unique_index('uq_ab_test_assignments_hash', 'ab_test_assignments', 
                          ['test_id', 'user_hash'])
    
    # Performance indexes
    op.create_index('idx_ab_test_assignments_test_variant', 'ab_test_assignments', 
                   ['test_id', 'variant_id'])
    op.create_index('idx_ab_test_assignments_conversion', 'ab_test_assignments', 
                   ['test_id', 'converted', 'converted_at'])
    op.create_index('idx_ab_test_assignments_user_hash', 'ab_test_assignments', 
                   ['user_hash', 'test_id'])
    op.create_index('idx_ab_test_assignments_session', 'ab_test_assignments', 
                   ['session_id', 'test_id'])
