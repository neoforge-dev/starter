...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('rec_metadata', postgresql.JSON(astext_type=sa.Text()), nullable=True),
        ...
        sa.UniqueConstraint('user_id', 'type', 'recommendation_id', name='uq_user_type_recommendation'),
    )
    
    # Create indexes for recommendations table
    op.create_index('idx_recommendations_user_status', 'recommendations', ['user_id', 'status'])
    op.create_index('idx_recommendations_user_type_status', 'recommendations', ['user_id', 'type', 'status'])
    op.create_index('idx_recommendations_priority_confidence', 'recommendations', ['priority_score', 'confidence_score'])
    op.create_index('idx_recommendations_created_expires', 'recommendations', ['created_at', 'expires_at'])
    op.create_index('idx_recommendations_performance', 'recommendations', ['type', 'status', 'created_at'])
    op.create_index('idx_recommendations_context_gin', 'recommendations', ['context'], postgresql_using='gin')
    op.create_index('idx_recommendations_metadata_gin', 'recommendations', ['rec_metadata'], postgresql_using='gin')
    ...
    op.create_index('ix_recommendations_relevance_score', 'recommendations', ['relevance_score'])
    ...
    op.create_table(
        ...
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('user_id', sa.Integer(), nullable=False),
        sa.Column('feature_interests', postgresql.JSON(astext_type=sa.Text()), nullable=True),
        ...
        sa.Column('behavioral_patterns', postgresql.JSON(astext_type=sa.Text()), nullable=True),
        sa.Column('avg_session_duration', sa.Float(), nullable=True),
        sa.Column('weekly_active_days', sa.Integer(), nullable=False, server_default=sa.text('0')),
        sa.Column('feature_adoption_rate', sa.Float(), nullable=False, server_default=sa.text('0.0')),
        sa.Column('notification_preferences', postgresql.JSON(astext_type=sa.Text()), nullable=True),
        sa.Column('max_daily_recommendations', sa.Integer(), nullable=False, server_default=sa.text('5')),
        sa.Column('created_at', sa.DateTime(), nullable=False, server_default=sa.text('CURRENT_TIMESTAMP')),
    ...
    op.create_index('idx_user_preferences_analyzed', 'user_preferences', ['last_analyzed_at'])
    op.create_index('idx_user_preferences_adoption', 'user_preferences', ['feature_adoption_rate'])
    op.create_index('idx_user_preferences_features_gin', 'user_preferences', ['feature_interests'], postgresql_using='gin')
    op.create_index('idx_user_preferences_content_gin', 'user_preferences', ['content_preferences'], postgresql_using='gin')
    op.create_index('idx_user_preferences_behavior_gin', 'user_preferences', ['behavioral_patterns'], postgresql_using='gin')
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('recommendation_id', 'user_id', name='uq_recommendation_user_feedback'),
    )
    
    # Create indexes for recommendation_feedback table
    op.create_index('idx_recommendation_feedback_rec_user', 'recommendation_feedback', ['recommendation_id', 'user_id'])
    op.create_index('idx_recommendation_feedback_type_action', 'recommendation_feedback', ['feedback_type', 'action_taken'])
    op.create_index('idx_recommendation_feedback_rating', 'recommendation_feedback', ['rating'])
    op.create_index('idx_recommendation_feedback_created', 'recommendation_feedback', ['created_at'])
    op.create_index('idx_recommendation_feedback_context_gin', 'recommendation_feedback', ['context'], postgresql_using='gin')
    ...
    op.create_table(
        ...
        sa.Column('common_features', postgresql.JSON(astext_type=sa.Text()), nullable=True),
    ...
    op.create_index('idx_similar_users_user_score', 'similar_users', ['user_id', 'similarity_score'])
    op.create_index('idx_similar_users_similarity_expires', 'similar_users', ['similar_user_id', 'expires_at'])
    op.create_index('idx_similar_users_score_computed', 'similar_users', ['similarity_score', 'computed_at'])
    ...
    op.create_index('idx_similar_users_features_gin', 'similar_users', ['common_features'], postgresql_using='gin')
    op.create_index('ix_similar_users_user_id', 'similar_users', ['user_id'])
    op.create_index('ix_similar_users_similar_user_id', 'similar_users', ['similar_user_id'])
    op.create_index('ix_similar_users_similarity_score', 'similar_users', ['similarity_score'])
