...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_user_sessions_hashed_refresh_token",
        "user_sessions",
    ...
    op.create_table(
        ...
        sa.Column(
            "status", sa.String(), nullable=False, server_default="open", index=True
        ),
        sa.Column(
            "created_at", sa.DateTime(), server_default=sa.text("CURRENT_TIMESTAMP")
        ),
    ...
    op.create_index(
        "ix_status_events_service_id", "status_events", ["service_id"], unique=False
    )
    op.create_index(
        "ix_status_events_status", "status_events", ["status"], unique=False
    )
    op.create_index(
        "ix_status_events_created_at", "status_events", ["created_at"], unique=False
    )
