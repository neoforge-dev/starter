...
def upgrade() -> None:
    ...
    role_type_enum = postgresql.ENUM(
        "SYSTEM", "CUSTOM", "INHERITED", name="roletype", create_type=False
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["parent_id"], ["organizations.id"], ondelete="CASCADE"
        ),
        sa.UniqueConstraint("uuid"),
    )
    ...
    op.create_index(
        "idx_org_tenant_slug", "organizations", ["tenant_id", "slug"], unique=True
    )
    op.create_index(
        "idx_org_tenant_parent", "organizations", ["tenant_id", "parent_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "conflicts_with", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
    ...
    op.create_index(
        "idx_user_role_tenant", "user_role_assignments", ["user_id", "tenant_id"]
    )
    op.create_index(
        "idx_user_role_org", "user_role_assignments", ["user_id", "organization_id"]
    )
    ...
    op.create_index(
        "idx_audit_tenant_action", "tenant_audit_logs", ["tenant_id", "action"]
    )
    op.create_index("idx_audit_actor", "tenant_audit_logs", ["actor_id"])
    op.create_index(
        "idx_audit_resource", "tenant_audit_logs", ["resource_type", "resource_id"]
    )
    ...
    op.create_index(
        "idx_role_audit_tenant_action", "role_audit_logs", ["tenant_id", "action"]
    )
