...
from app.models.content_suggestion import ContentCategory, ContentItem, ContentType
...
class AIContentAnalyzer:
    ...
    async def analyze_content_item(
        ...
        try:
            ...
            if "quality" in analysis_types:
                analysis_results["quality"] = await self._analyze_quality(
            ...
            if "sentiment" in analysis_types:
                analysis_results["sentiment"] = await self._analyze_sentiment(
                    content_text
                )
            ...
            if "engagement" in analysis_types:
                analysis_results["engagement"] = await self._predict_engagement(
                    content_text, content_item
                )
            ...
            engagement_prediction = analysis_results.get("engagement", {}).get(
                "prediction", 0.5
            )
    ...
    async def optimize_content_for_engagement(
        ...
        try:
            ...
            return {
                ...
                "improvement_score": optimization_results.get(
                    "improvement_potential", 0.5
                ),
                "implementation_complexity": optimization_results.get(
                    "complexity", "medium"
                ),
            ...
            logger.error(
                f"Error optimizing content {content_item.content_id}: {str(e)}"
            )
            return {
                "content_id": content_item.content_id,
                "error": str(e),
    ...
    async def _calculate_content_relevance(
        ...
        try:
            ...
            category_pref = content_prefs.get("categories", {}).get(
                content_item.category, 0.5
            )

            # Type preference boost
            type_pref = content_prefs.get("types", {}).get(
            ...
            if content_item.topics and user_context.get("interests"):
                ...
                if content_topics and user_interests:
                    ...
                    overlap_score = len(
                        set(content_topics) & set(user_interests.keys())
                    ) / max(len(content_topics), 1)
    ...
    async def _calculate_suggestion_confidence(
        ...
        try:
            ...
            if (
                ...
                and (datetime.utcnow() - content_item.created_at).days < 30
            ):
    ...
    def _calculate_personalization_score(
        ...
        try:
            ...
            if content_item.topics:
                topic_overlap = len(
                    set(content_item.topics.get("main_topics", []))
                    & set(interests.keys())
                )
                if topic_overlap > 0:
            ...
            ) == "short" and content_item.content_type in ["tutorial", "article"]:
                personalization_factors.append(0.1)

            total_personalization = sum(personalization_factors)
            return min(
                max(total_personalization, 0.1), 1.0
            )  # Minimum 0.1 to indicate some personalization
        ...
        try:
            ...
            return {
                ...
                "ai_reasoning": {
                    ...
                    "personalization_factors": result.get(
                        "personalization_factors", []
                    ),
