...
class EventType(str, Enum):
    ...
    INTERACTION = "interaction"     # User interactions (clicks, navigation, form submissions)
...
class EventCreateBulk(BaseModel):
    ...
    events: List[EventCreate] = Field(..., min_items=1, max_items=100, description="Events to create")
...
class EventAnalyticsQuery(BaseModel):
    ...
    aggregate_functions: Optional[List[str]] = Field(["count"], description="Aggregate functions")
...
class EventAggregateResult(BaseModel):
    ...
    timestamp_range: Dict[str, Optional[datetime]] = Field(..., description="Time range")
...
class EventStreamConfig(BaseModel):
    ...
    event_types: Optional[List[EventType]] = Field(None, description="Event types to stream")
...
class EventAnonymizationRequest(BaseModel):
    ...
    event_types: Optional[List[EventType]] = Field(None, description="Event types to anonymize")
    before_date: Optional[datetime] = Field(None, description="Anonymize events before date")
...
class EventRetentionPolicy(BaseModel):
    ...
    retention_days: int = Field(..., gt=0, le=2555, description="Retention period in days")  # Max ~7 years
    auto_anonymize: bool = Field(default=False, description="Auto-anonymize before deletion")
...
class EventDataExportRequest(BaseModel):
    ...
    event_types: Optional[List[EventType]] = Field(None, description="Event types to export")
    ...
    include_anonymized: bool = Field(default=False, description="Include anonymized events")