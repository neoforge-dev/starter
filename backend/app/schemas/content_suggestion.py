...
class ContentItem(ContentItemBase):
    ...
    quality_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="AI quality score"
    )
    relevance_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Relevance score"
    )
    engagement_prediction: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Predicted engagement"
    )
    ...
    optimization_suggestions: Optional[Dict[str, Any]] = Field(
...
class ContentSuggestionBase(BaseModel):
    ...
    title: str = Field(
        ..., min_length=1, max_length=300, description="Suggestion title"
    )
    description: str = Field(..., min_length=1, description="Suggestion description")
    call_to_action: Optional[str] = Field(
        None, max_length=100, description="Call to action"
    )
...
class ContentSuggestionCreate(ContentSuggestionBase):
    ...
    personalization_score: float = Field(
        ..., ge=0.0, le=1.0, description="Personalization score"
    )
    ...
    personalization_factors: Optional[Dict[str, Any]] = Field(
        None, description="Personalization factors"
    )
...
class ContentSuggestion(ContentSuggestionBase):
    ...
    created_at: datetime = Field(..., description="Creation timestamp")
    expires_at: Optional[datetime] = Field(None, description="Expiration timestamp")
    first_shown_at: Optional[datetime] = Field(
        None, description="First shown timestamp"
    )
    ...
    content_item: Optional[ContentItem] = Field(
        None, description="Associated content item"
    )
...
class ContentSuggestionFeedbackBase(BaseModel):
    ...
    rating: Optional[int] = Field(
        None, ge=1, le=5, description="User rating (1-5 stars)"
    )
...
class PersonalizedSuggestionsRequest(BaseModel):
    ...
    suggestion_types: Optional[List[SuggestionType]] = Field(
        None, description="Types of suggestions"
    )
    categories: Optional[List[ContentCategory]] = Field(
        None, description="Content categories"
    )
...
class GenerateContentSuggestionsRequest(BaseModel):
    ...
    content_ids: Optional[List[str]] = Field(
        None, description="Specific content IDs to analyze"
    )
    user_ids: Optional[List[int]] = Field(
        None, description="Specific user IDs to generate for"
    )
    categories: Optional[List[ContentCategory]] = Field(
        None, description="Content categories"
    )
    force_regenerate: bool = Field(
        False, description="Force regeneration of existing suggestions"
    )
    max_suggestions_per_user: int = Field(
        5, ge=1, le=20, description="Max suggestions per user"
    )
...
class TrendingContentRequest(BaseModel):
    ...
    min_engagement_rate: float = Field(
        0.01, ge=0.0, le=1.0, description="Minimum engagement rate"
    )
...
class TrendingContentResponse(BaseModel):
    ...
    trending_content: List[ContentItem] = Field(
...
class ContentOptimizationRequest(BaseModel):
    ...
    optimization_types: Optional[List[str]] = Field(
        None, description="Types of optimization"
    )
...
class ContentSuggestionAnalyticsRequest(BaseModel):
    ...
    suggestion_types: Optional[List[SuggestionType]] = Field(
        None, description="Suggestion types"
    )
...
class ContentSuggestionAnalyticsResponse(BaseModel):
    ...
    top_performing_types: List[Dict[str, Any]] = Field(
        ..., description="Top performing suggestion types"
    )
    ...
    time_series_data: Optional[List[Dict[str, Any]]] = Field(
        None, description="Time series analytics"
    )
...
class BulkContentSuggestionCreate(BaseModel):
    ...
    suggestions: List[ContentSuggestionCreate] = Field(
        ..., description="List of suggestions to create"
    )
    notify_users: bool = Field(
        False, description="Whether to notify users of new suggestions"
    )
...
class BulkContentSuggestionResponse(BaseModel):
    ...
    errors: Optional[List[str]] = Field(
        None, description="Error messages for failed creations"
    )
...
class UserContentPreferencesUpdate(BaseModel):
    ...
    preferred_categories: Optional[List[ContentCategory]] = Field(
        None, description="Preferred categories"
    )
    content_types: Optional[List[ContentType]] = Field(
        None, description="Preferred content types"
    )
    notification_frequency: Optional[str] = Field(
        None, description="Notification frequency"
    )
    max_daily_suggestions: Optional[int] = Field(
        None, ge=1, le=50, description="Max daily suggestions"
    )
    quality_threshold: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Quality threshold"
    )
...
class UserContentPreferences(BaseModel):
    ...
    user_id: int = Field(..., description="User ID")
    preferred_categories: List[ContentCategory] = Field(
