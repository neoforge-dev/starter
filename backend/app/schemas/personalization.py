...
class PersonalizationProfileBase(PersonalizationBase):
    ...
    secondary_segments: Optional[List[str]] = Field(None, description="Additional user segments")
    segment_confidence: float = Field(0.5, ge=0.0, le=1.0, description="Segment assignment confidence")
    usage_patterns: Optional[Dict[str, Any]] = Field(None, description="Usage patterns analysis")
    feature_usage: Optional[Dict[str, Any]] = Field(None, description="Feature usage patterns")
    navigation_patterns: Optional[Dict[str, Any]] = Field(None, description="Navigation preferences")
    device_preferences: Optional[Dict[str, Any]] = Field(None, description="Device usage patterns")
    ui_preferences: Optional[Dict[str, Any]] = Field(None, description="UI customization preferences")
...
class PersonalizationRuleBase(PersonalizationBase):
    ...
    target_segments: List[str] = Field(..., description="User segments this rule applies to")
    target_contexts: List[str] = Field(..., description="Contexts where rule is applied")
    ...
    configuration: Dict[str, Any] = Field(..., description="Personalization configuration")
...
class PersonalizationRuleCreate(PersonalizationRuleBase):
    ...
    def validate_personalization_type(cls, v):
        ...
        if v not in valid_types:
            raise ValueError(f'Invalid personalization type. Must be one of: {valid_types}')
        return v
...
class PersonalizationRequest(PersonalizationBase):
    ...
    device_type: Optional[str] = Field(None, description="Device type")
    session_id: Optional[str] = Field(None, description="Session identifier")
    include_ab_tests: bool = Field(True, description="Whether to include A/B test rules")
...
class RuleOptimizationRequest(PersonalizationBase):
    ...
    rule_ids: Optional[List[str]] = Field(None, description="Specific rules to optimize")
    segments: Optional[List[str]] = Field(None, description="Segments to optimize for")
    min_interactions: int = Field(100, ge=10, description="Minimum interactions for optimization")
...
class BulkInteractionCreate(PersonalizationBase):
    ...
    interactions: List[PersonalizationInteractionCreate] = Field(
        ...
        min_items=1, 
        max_items=1000,
        description="List of interactions to create"
    )
