...
class AbTestVariantBase(BaseModel):
    ...
    variant_key: str = Field(
        ..., min_length=1, max_length=50, description="Unique variant identifier"
    )
    name: str = Field(
        ..., min_length=1, max_length=200, description="Human-readable variant name"
    )
    ...
    def validate_variant_key(cls, v):
        ...
        if not v.replace("_", "").replace("-", "").isalnum():
            raise ValueError(
                "Variant key must contain only alphanumeric characters, hyphens, and underscores"
            )
        return v.lower()
...
class AbTestBase(BaseModel):
    ...
    test_key: str = Field(
        ..., min_length=1, max_length=100, description="Unique test identifier"
    )
    name: str = Field(
        ..., min_length=1, max_length=200, description="Human-readable test name"
    )
    description: Optional[str] = Field(
        None, max_length=5000, description="Test description and hypothesis"
    )
    traffic_allocation: float = Field(
        1.0, ge=0.0, le=1.0, description="Percentage of users to include"
    )
    primary_metric: str = Field(
        ..., min_length=1, max_length=100, description="Primary conversion metric"
    )
    secondary_metrics: Optional[Dict[str, Any]] = Field(
        None, description="Additional metrics to track"
    )
    confidence_level: float = Field(
        0.95, ge=0.8, le=0.99, description="Required confidence level"
    )
    minimum_detectable_effect: float = Field(
        0.05, ge=0.01, le=0.5, description="Minimum effect size"
    )
    minimum_sample_size: int = Field(
        1000, ge=100, le=1000000, description="Minimum sample size per variant"
    )
    targeting_rules: Optional[Dict[str, Any]] = Field(
        None, description="User targeting rules"
    )
    ...
    def validate_test_key(cls, v):
        ...
        if not v.replace("_", "").replace("-", "").isalnum():
            raise ValueError(
                "Test key must contain only alphanumeric characters, hyphens, and underscores"
            )
        return v.lower()
...
class AbTestCreate(AbTestBase):
    ...
    variants: List[AbTestVariantCreate] = Field(
        ..., min_items=2, max_items=10, description="Test variants"
    )
...
class AbTestAnalyticsQuery(BaseModel):
    ...
    test_ids: Optional[List[int]] = Field(
        None, description="Specific test IDs to analyze"
    )
    test_keys: Optional[List[str]] = Field(
        None, description="Specific test keys to analyze"
    )
    status_filter: Optional[List[AbTestStatus]] = Field(
    ...
    statistical_method: StatisticalMethod = Field(
        StatisticalMethod.FREQUENTIST, description="Statistical method"
    )
