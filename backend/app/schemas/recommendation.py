...
class RecommendationBase(BaseModel):
    ...
    priority_score: float = Field(
        ..., ge=0.0, le=1.0, description="Business priority score"
    )
    relevance_score: float = Field(
        ..., ge=0.0, le=1.0, description="User relevance score"
    )
    context: Optional[Dict[str, Any]] = Field(
        None, description="Additional context data"
    )
    rec_metadata: Optional[Dict[str, Any]] = Field(
...
class RecommendationCreateBulk(BaseModel):
    ...
    recommendations: List[RecommendationCreate] = Field(
        ..., min_items=1, max_items=100, description="List of recommendations to create"
    )
...
class RecommendationBulkResponse(BaseModel):
    ...
    recommendations: List[Recommendation] = Field(
        ..., description="Created recommendations"
    )
...
class UserPreferencesBase(BaseModel):
    ...
    feature_interests: Optional[Dict[str, float]] = Field(
        None, description="Feature interest scores"
    )
    ...
    behavioral_patterns: Optional[Dict[str, Any]] = Field(
        None, description="Behavioral patterns"
    )
    avg_session_duration: Optional[float] = Field(
        None, ge=0.0, description="Avg session duration (minutes)"
    )
    weekly_active_days: int = Field(0, ge=0, le=7, description="Active days per week")
    feature_adoption_rate: float = Field(
        0.0, ge=0.0, le=1.0, description="Feature adoption rate"
    )
    ...
    max_daily_recommendations: int = Field(
        5, ge=1, le=50, description="Max daily recommendations"
    )
...
class SimilarUsersBase(BaseModel):
    ...
    algorithm: str = Field(..., max_length=50, description="Algorithm used")
    behavioral_similarity: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Behavioral similarity"
    )
    preference_similarity: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Preference similarity"
    )
    demographic_similarity: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Demographic similarity"
    )
    common_features: Optional[Dict[str, Any]] = Field(
        None, description="Common features"
    )
...
class RecommendationRequest(BaseModel):
    ...
    types: Optional[List[str]] = Field(
        None, description="Recommendation types to include"
    )
    ...
    exclude_dismissed: bool = Field(
        True, description="Exclude dismissed recommendations"
    )
...
class RecommendationAnalytics(BaseModel):
    ...
    top_performing_types: List[Dict[str, Any]] = Field(
        ..., description="Top performing types"
    )
...
class ModelRetrainingRequest(BaseModel):
    ...
    max_training_time: int = Field(
        600, ge=60, le=3600, description="Max training time in seconds"
    )
