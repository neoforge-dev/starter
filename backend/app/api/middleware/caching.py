...
class ResponseCachingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if cached_response:
            ...
            cached_response.headers["X-Process-Time"] = str(round(process_time * 1000, 2))
    ...
    async def _get_cached_response(
        ...
        try:
            ...
            response_data = json.loads(cached_data) if isinstance(cached_data, str) else cached_data
    ...
    async def _cache_response(
        ...
        try:
            ...
            if hasattr(response, 'body'):
                try:
                    content = json.loads(response.body.decode()) if response.body else None
...
def setup_caching_middleware(
    ...
    if enable_middleware and settings.environment in [Environment.PRODUCTION, Environment.STAGING]:
        logger.info(
        ...
        logger.info(
            ...
            reason="development_environment" if not enable_middleware else "explicit_disable",
            environment=settings.environment.value
        )