...
class SecurityValidator:
    ...
    def is_suspicious_path(path: str) -> bool:
        ...
        return any(
            ...
            for suspicious in SecurityValidator.SUSPICIOUS_PATHS
        )
...
class RequestValidationMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, settings: Settings):
        ...
        if self.is_production:
            self.required_headers = {"Accept", "User-Agent", "Accept-Encoding"}
        else:
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if not self.security_validator.validate_user_agent(user_agent):
                ...
                return JSONResponse(
                    status_code=400, content={"detail": "Invalid User-Agent"}
                )
            ...
            if endpoint.startswith(self.settings.api_v1_str):
                ...
                if method in ["POST", "PUT", "PATCH"]:
                    try:
                        ...
                        if (
                            content_length
                            and int(content_length) > SecurityValidator.MAX_JSON_SIZE
                        ):
                            return JSONResponse(
                                status_code=413,
                        ...
                        if self.is_production:
                            security_threats = (
                                self.security_validator.validate_input_security(body)
                            )
                            if security_threats:
                                logger.warning(
...
def setup_validation_middleware(app: FastAPI) -> None:
    ...
    current_settings = (
        get_settings()
    )  # Fetch settings temporarily for standalone use potential
