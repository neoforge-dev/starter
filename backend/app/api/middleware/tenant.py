...
class TenantMiddleware(BaseHTTPMiddleware):
    ...
    async def _resolve_from_headers(self, request: Request) -> TenantContext:
        ...
        if tenant_id:
            try:
                ...
                if tenant:
                    return TenantContext(tenant=tenant, resolved_from="header_id")
            except Exception as e:
        ...
        if tenant_slug:
            try:
                ...
                if tenant:
                    return TenantContext(tenant=tenant, resolved_from="header_slug")
            except Exception as e:
    ...
    async def _resolve_from_domain(self, request: Request) -> TenantContext:
        ...
        if tenant:
            return TenantContext(
                tenant=tenant, domain=host, resolved_from="custom_domain"
            )
        ...
        if len(host_parts) >= 3:
            ...
            if subdomain not in {"www", "api", "app", "admin"}:
                ...
                if tenant:
                    return TenantContext(
                        tenant=tenant, subdomain=subdomain, resolved_from="subdomain"
                    )
    ...
    async def _get_cached_tenant(
        self, cache_key: str, field: str, value: str
    ) -> Optional[Tenant]:
    ...
    async def _validate_tenant_access(
        ...
        if tenant.status == TenantStatus.SUSPENDED:
            ...
            raise HTTPException(
                status_code=status.HTTP_410_GONE, detail="Tenant is cancelled"
            )
