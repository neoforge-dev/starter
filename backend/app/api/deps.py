...
async def get_db(request: Request) -> AsyncGenerator[AsyncSession, None]:
    ...
    if hasattr(request.app.state, "_test_session") and request.app.state._test_session:
        logger.debug(
            f"Using test session {id(request.app.state._test_session)} from app.state"
        )
...
async def get_current_active_superuser(
    ...
    ),  # This dependency chain is now correct
) -> User:
    """Get current active superuser."""
    logger = logging.getLogger(__name__)
    logger.debug(
        f"Checking superuser status for user: {current_user.id}, is_superuser: {current_user.is_superuser}"
    )
...
async def get_current_admin(
    ...
    logger = logging.getLogger(__name__)
    logger.debug(
        f"Attempting to get admin for user_id: {current_user.id}"
    )  # Now current_user is a User object

    try:
        admin = await crud.admin.get_by_user_id(db=db, user_id=current_user.id)
        if not admin:
            logger.debug(
                f"No admin found for user_id: {current_user.id} - raising 403 FORBIDDEN"
            )
            raise HTTPException(
        ...
        if not admin.is_active:
            logger.debug(
                f"Admin found but not active for user_id: {current_user.id} - raising 403 FORBIDDEN"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        logger.debug(
            f"Admin found for user_id: {current_user.id}, admin_id: {admin.id}, role: {admin.role}"
        )
        return admin
    except HTTPException as e:
        logger.debug(
            f"HTTPException in get_current_admin: status_code={e.status_code}, detail={e.detail}"
        )
        raise
...
async def get_monitored_db(request: Request) -> AsyncGenerator[QueryMonitor, None]:
    ...
    if hasattr(request.app.state, "_test_session") and request.app.state._test_session:
        logger.debug(
            f"Using test session {id(request.app.state._test_session)} from app.state for monitored DB"
        )
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if (
                    hasattr(monitor, "current_query")
                    and monitor.current_query
                    and "health" in str(monitor.current_query).lower()
                ):
                    raise HTTPException(
