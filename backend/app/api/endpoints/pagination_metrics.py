...
async def get_pagination_metrics(
    hours: int = Query(
        24, ge=1, le=168, description="Hours of metrics to analyze (1-168)"
    ),
    ...
    **Optimization Recommendations:**
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve pagination metrics: {str(e)}"
        )
    ...
    try:
        ...
        if "summary" in recent_summary:
            ...
            if p95_duration > 500:  # Critical threshold
                ...
                health_status = (
                    "degraded" if health_status == "healthy" else health_status
                )
        ...
        active_queries = (
            monitor.active_queries_gauge._value._value
            if hasattr(monitor.active_queries_gauge, "_value")
            else 0
        )
        ...
        return {
            ...
            "alerts": alerts,
            "recent_performance": {
                "queries_last_hour": recent_summary.get("summary", {}).get(
                    "total_queries", 0
                ),
                "p95_response_time_ms": recent_summary.get("summary", {}).get(
                    "p95_duration_ms", 0
                ),
                "cache_hit_rate": recent_summary.get("summary", {}).get(
                    "cache_hit_rate", 0
                ),
                "slow_query_count": recent_summary.get("summary", {}).get(
                    "slow_queries", 0
                ),
            },
            "sla_compliance": {
                "target_ms": 200,
                "current_p95_ms": recent_summary.get("summary", {}).get(
                    "p95_duration_ms", 0
                ),
                "meets_sla": recent_summary.get("summary", {}).get("p95_duration_ms", 0)
                <= 200,
            },
        }
...
async def get_optimization_recommendations(
    hours: int = Query(
        24, ge=1, le=168, description="Hours of data to analyze for recommendations"
    ),
    monitor: PaginationPerformanceMonitor = Depends(get_performance_monitor),
) -> Dict[str, Any]:
    ...
    **Analysis Factors:**
    ...
    try:
        ...
        return {
            ...
            "performance_summary": {
                "total_queries": performance_summary.get("summary", {}).get(
                    "total_queries", 0
                ),
                "avg_duration_ms": performance_summary.get("summary", {}).get(
                    "avg_duration_ms", 0
                ),
                "cache_hit_rate": performance_summary.get("summary", {}).get(
                    "cache_hit_rate", 0
                ),
            },
        }
...
def _determine_recommendation_priority(
    recommendation: str, performance_data: Dict[str, Any]
) -> str:
