...
from app.utils.memory_optimization import force_garbage_collection, get_memory_report
...
async def get_performance_metrics(
    include_db_stats: bool = Query(
        False, description="Include database performance stats"
    ),
...
async def clear_cache(
    pattern: Optional[str] = Query(
        None, description="Pattern to match keys for deletion"
    )
) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "cost_optimization": {
                ...
                "next_priority": recommendations[0]["category"]
                if recommendations
                else "none",
            },
...
def _calculate_cost_optimization(perf_stats: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "estimated_monthly_savings_usd": monthly_bandwidth_savings
        + monthly_compute_savings,
        "optimization_score": _calculate_optimization_score(
            compression_stats, dedup_stats
        ),
    }
...
def _estimate_monthly_savings(
    perf_stats: Dict[str, Any], cache_stats: Dict[str, Any]
) -> float:
    ...
    compression_savings = (
        perf_stats.get("compression", {}).get("bytes_saved", 0) * 0.0001
    )
...
async def get_memory_performance() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get memory metrics: {str(e)}"
        )
...
async def trigger_memory_gc() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to trigger garbage collection: {str(e)}"
        )
...
async def get_database_performance(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get database performance: {str(e)}"
        )
...
async def get_comprehensive_performance(
    ...
    try:
        ...
        memory_score = max(
            0, 100 - (memory_stats["current"]["process_memory_percent"] - 50)
        )
        ...
        return {
            ...
            "cache_performance": {"stats": cache_stats, "score": round(cache_score, 1)},
            "memory_performance": {
                "stats": memory_stats,
