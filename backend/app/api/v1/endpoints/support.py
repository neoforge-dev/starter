...
async def create_ticket(
    ...
    await idem.store(
        SupportTicketRead.model_validate(created).model_dump(), status_code=201
    )
    return created


@router.get(
    "/support/tickets", response_model=Union[PaginatedResponse, CursorPaginatedResponse]
)
async def list_tickets(
    ...
    sort_by: str = Query(
        "created_at", description="Field to sort by (created_at, updated_at, status)"
    ),
    ...
    status: Optional[str] = Query(
        None, description="Filter by ticket status (open, closed, pending)"
    ),
    email_like: Optional[str] = Query(
        None, description="Search tickets by email (partial match)"
    ),
    subject_like: Optional[str] = Query(
        None, description="Search tickets by subject (partial match)"
    ),
    ...
    **Common Use Cases:**
    ...
    if use_offset_pagination:
        ...
        items, total = await st_crud.get_multi_with_count(
        ...
        ticket_data = [
            SupportTicketRead.model_validate(ticket) for ticket in paginated_result.data
        ]
