...
from app.api.middleware.tenant import TenantContext, get_tenant_context, require_tenant
...
class OrganizationCRUD(
    OrganizationScopedCRUD[Organization, OrganizationCreate, OrganizationUpdate]
):
    ...
    try:
        ...
        if search:
            ...
            organizations = [
                ...
                if search_lower in org.name.lower()
                or search_lower in org.slug.lower()
                or (org.description and search_lower in org.description.lower())
            ]
    ...
    try:
        ...
        if org_in.parent_id is not None and org_in.parent_id != organization.parent_id:
            ...
            if await _would_create_cycle(
                db, organization_id, org_in.parent_id, tenant_context.tenant_id
            ):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
        ...
        logger.error(
            f"Failed to update organization {organization_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def delete_organization(
    *,
    ...
    try:
        ...
        if (
            member_count > 1 and not force
        ):  # Allow deletion if only current user is member
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        logger.error(
            f"Failed to delete organization {organization_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.get(
    "/{organization_id}/hierarchy", response_model=OrganizationHierarchyResponse
)
...
@router.get(
    "/{organization_id}/members", response_model=List[OrganizationMembershipResponse]
)
...
async def _add_organization_member(
    ...
    role_result = await db.execute(
        select(Role).where(and_(Role.name == role_name, Role.tenant_id == tenant_id))
    )
...
async def _would_create_cycle(
    db: AsyncSession, org_id: int, new_parent_id: int, tenant_id: int
) -> bool:
    ...
    while current_id:
        ...
        result = await db.execute(
            select(Organization.parent_id).where(
                and_(Organization.id == current_id, Organization.tenant_id == tenant_id)
            )
        )
...
async def _get_organization_ancestors(
    db: AsyncSession, org_id: int, tenant_id: int, max_depth: int
) -> List[Organization]:
