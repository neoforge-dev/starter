...
async def recompute_user_profile(
    ...
    try:
        profile = await personalization_engine.update_user_profile(
            db, user_id=user_id, force_recompute=True
        )
...
async def get_personalized_configuration(
    ...
    try:
        config = await personalization_engine.get_personalized_config(
            db, request=request
        )
        return config
        ...
        logger.error(
            f"Error generating personalized config for user {request.user_id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def track_bulk_personalization_interactions(
    ...
    try:
        ...
        return {
            "message": f"Tracked {len(interactions.interactions)} interactions successfully"
        }
...
async def get_user_interactions(
    ...
    try:
        interactions = await personalization_interaction.get_user_interactions(
            db, user_id=user_id, context=context, days=days, skip=skip, limit=limit
        )
...
async def get_personalization_analytics(
    ...
    try:
        ...
        conversion_improvement = (
            15.0  # Placeholder - would be calculated from A/B test data
        )
        ...
        return PersonalizationAnalytics(
            ...
            active_rules=len(
                [
                    r
                    for r in await personalization_rule.get_multi(db, limit=1000)
                    if r.is_active
                ]
            ),
...
async def get_user_personalization_insights(
    ...
    try:
        insights_data = await personalization_engine.get_user_insights(
            db, user_id=user_id
        )

        if "error" in insights_data:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=insights_data["error"]
            )
        ...
        return PersonalizationInsights(
            ...
            predicted_actions=[
                ...
                for action in insights_data.get("ml_insights", {}).get(
                    "next_actions", []
                )
            ],
            ...
            churn_risk_factors=insights_data.get("ml_insights", {}).get(
                "churn_factors", []
            ),
