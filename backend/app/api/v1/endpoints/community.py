...
@router.get(
    "/community/posts", response_model=Union[PaginatedResponse, CursorPaginatedResponse]
)
async def list_posts(
    ...
    sort_by: str = Query(
        "created_at", description="Field to sort by (created_at, updated_at, title)"
    ),
    ...
    author_like: Optional[str] = Query(
        None, description="Search posts by author (partial match)"
    ),
    title_like: Optional[str] = Query(
        None, description="Search posts by title (partial match)"
    ),
    content_like: Optional[str] = Query(
        None, description="Search posts by content (partial match)"
    ),
    ...
    **Common Use Cases:**
    ...
    if use_offset_pagination:
        ...
        items, total = await cp_crud.get_multi_with_count(
        ...
        post_data = [
            CommunityPostRead.model_validate(post) for post in paginated_result.data
        ]
...
async def create_post(
    ...
    await idem.store(
        CommunityPostRead.model_validate(created).model_dump(), status_code=201
    )
    return created
