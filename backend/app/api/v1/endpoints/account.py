...
async def deactivate_account(
    ...
    try:
        ...
        deactivated_user = await user_crud.deactivate_account(
            db=db, user_id=current_user.id, reason=request.reason
        )
...
async def reactivate_account(
    ...
    if not current_user.is_account_deactivated():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Account is not deactivated"
        )
...
async def schedule_account_deletion(
    ...
    try:
        ...
        scheduled_user = await user_crud.schedule_deletion(
            db=db, user_id=current_user.id, retention_days=request.retention_days
        )
        ...
        await db.rollback()
        logger.error(
            f"Account deletion scheduling failed for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to schedule account deletion",
...
async def cancel_account_deletion(
    ...
    try:
        # Cancel deletion
        updated_user = await user_crud.cancel_deletion(db=db, user_id=current_user.id)
        ...
        await db.rollback()
        logger.error(
            f"Account deletion cancellation failed for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to cancel account deletion",
...
async def change_password(
    ...
    if not verify_password(request.current_password, current_user.hashed_password):
        ...
        await security_auditor.log_security_event(
            ...
            user_id=current_user.id,
            severity=SecuritySeverity.MEDIUM,
            details={"result": "failed", "reason": "incorrect_current_password"},
            success=False,
        )
    ...
    try:
        ...
        updated_user = await user_crud.update(
            db=db, db_obj=current_user, obj_in={"password": request.new_password}
        )

        # Update password change timestamp
        await user_crud.update_password_changed(db=db, user_id=current_user.id)
        ...
        logger.info(
            "password_changed", user_id=current_user.id, revoked_sessions=revoked_tokens
        )
...
async def get_account_status(
    ...
    return {
        ...
        "locked_until": current_user.locked_until.isoformat()
        ...
        "deletion_date": current_user.data_retention_until.isoformat()
        if current_user.data_retention_until
        else None,
