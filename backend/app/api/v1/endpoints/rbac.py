...
from app.crud.tenant_crud import GlobalCRUD, TenantCRUDBase
...
async def get_role(
    *,
    ...
    query = select(Role).where(
        and_(Role.id == role_id, Role.tenant_id == tenant_context.tenant_id)
    )
    ...
    if include_hierarchy:
        query = query.options(
            selectinload(Role.parent_role), selectinload(Role.child_roles)
        )
    ...
    *,
    ...
    try:
        ...
        role = await role_crud.get_by_id_with_tenant(
            db, id=role_id, tenant_id=tenant_context.tenant_id, user_id=current_user.id
        )
    ...
    *,
    ...
    try:
        ...
        user_count = await db.execute(
            select(func.count())
            .select_from(user_role_assignments)
            .where(user_role_assignments.c.role_id == role_id)
        )
...
async def assign_role(
    *,
    ...
    try:
        ...
        existing = await db.execute(
            ...
            .where(
                and_(
                    ...
                    user_role_assignments.c.organization_id
                    == assignment_in.organization_id,
                )
            )
        )
        ...
        await db.execute(user_role_assignments.insert().values(**assignment_data))
...
async def _assign_permissions_to_role(
    db: AsyncSession, role_id: int, permission_ids: List[int], tenant_id: int
):
    ...
    await db.execute(role_permissions.insert().values(assignments))


async def _invalidate_role_permission_cache(
    db: AsyncSession, role_id: int, tenant_id: int
):
...
async def _invalidate_user_permission_cache(
    ...
    query = delete(PermissionCache).where(
        and_(PermissionCache.user_id == user_id, PermissionCache.tenant_id == tenant_id)
    )
