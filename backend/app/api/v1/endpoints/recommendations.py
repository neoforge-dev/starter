    ...
    *,
    ...
    try:
        ...
        existing_recommendations = (
            await crud_recommendation.recommendation.get_user_recommendations(
                db,
                user_id=user_id,
                types=types,
                statuses=statuses,
        ...
        user_prefs = await crud_recommendation.user_preferences.get_by_user_id(
            db, user_id=user_id
        )
        user_preferences_data = (
            UserPreferences.model_validate(user_prefs) if user_prefs else None
        )
        ...
        logger.info(
            f"Generated {len(recommendations_data)} recommendations for user {user_id}"
        )
        return response
...
async def get_trending_recommendations(
    *,
    ...
    try:
        cache = get_cache()
        cache_key = (
            f"trending:types:{':'.join(types or [])}:window:{time_window}:limit:{limit}"
        )
        ...
        trending_data = (
            await crud_recommendation.recommendation.get_trending_recommendations(
                db,
                types=types,
...
async def submit_recommendation_feedback(
    *,
    ...
    try:
        ...
        recommendation = (
            await crud_recommendation.recommendation.get_by_recommendation_id(
        ...
        logger.info(
            f"User {current_user.id} provided feedback on recommendation {feedback_in.recommendation_id}"
        )
...
async def get_similar_users(
    *,
    ...
    try:
        ...
        # Convert to response format
        similar_users_list = [
            crud_recommendation.similar_users.model_validate(su)
        ...
        logger.info(
            f"Retrieved {len(similar_users_list)} similar users for user {user_id}"
        )
        return response
...
async def retrain_recommendation_models(
    *,
    ...
    try:
        ...
        logger.info(
            f"Model retraining started by user {current_user.id} with version {model_version}"
        )
        return response
...
async def get_recommendation_analytics(
    *,
    ...
    try:
        ...
        # Get analytics data
        analytics_data = (
            await crud_recommendation.recommendation.get_performance_analytics(
                db,
                user_id=user_id,
...
async def _retrain_models_background(
    ...
    try:
        ...
        expired_cleaned = (
            await crud_recommendation.similar_users.cleanup_expired_similarities(db)
        )
