...
from app.models.recommendation import Recommendation, UserPreferences, SimilarUsers
from app.schemas.recommendation import (
    RecommendationCreate, UserPreferencesUpdate, SimilarUsersCreate,
...
class RecommendationEngine:
    ...
    async def _hybrid_filtering(
        ...
        collaborative_recs = await self._collaborative_filtering(user_id, rec_type, user_prefs)
        content_based_recs = await self._content_based_filtering(user_id, rec_type, user_prefs)
        ...
        for rec_data in all_recs.values():
            ...
            hybrid_score = (
                0.6 * rec_data["collaborative_score"] + 
                0.4 * rec_data["content_score"]
            )
    ...
    async def _recommend_features_collaborative(
        ...
        for feature, data in sorted(feature_scores.items(), key=lambda x: x[1]["score"], reverse=True)[:3]:
            ...
            recommendations.append(RecommendationCreate(
                ...
                context={"feature_name": feature, "similar_users_count": len(data["users"])},
                ...
                for suggested_feature in feature_suggestions[used_feature]:
                    if suggested_feature not in used_features_set:
                        ...
                        recommendations.append(RecommendationCreate(
                            ...
                            context={"base_feature": used_feature, "suggested_feature": suggested_feature},
                            model_version=self.model_version,
    ...
    async def _recommend_actions_collaborative(
        ...
        if user_recent_actions:
            ...
            relevant_patterns = [
                (pattern, count) for pattern, count in action_patterns.items()
                if pattern.startswith(f"{last_action} -> ")
            ]
            
            for pattern, count in sorted(relevant_patterns, key=lambda x: x[1], reverse=True)[:2]:
                ...
                recommendations.append(RecommendationCreate(
                    ...
                    priority_score=0.8,
                    relevance_score=confidence,
                    context={"last_action": last_action, "suggested_action": next_action, "pattern_strength": count},
                    model_version=self.model_version,
                    algorithm="collaborative",
                ))
        ...
        for workflow in incomplete_workflows:
            ...
            if workflow in completion_steps:
                ...
                recommendations.append(RecommendationCreate(
                    ...
                    relevance_score=confidence,
                    context={"incomplete_workflow": workflow, "next_step": next_step},
                    model_version=self.model_version,
                    algorithm="content_based",
                ))
    ...
    async def _recommend_content_collaborative(
        ...
        for topic, score in sorted(content_interests.items(), key=lambda x: x[1], reverse=True)[:3]:
        ...
        for consumed_content, score in content_interests.items():
            if consumed_content in content_suggestions and score > 0.3:
                for suggested_content in content_suggestions[consumed_content]:
                    confidence = score * 0.8  # Slightly lower confidence than direct interest
                    ...
                    recommendations.append(RecommendationCreate(
                        ...
                        priority_score=0.5,
                        relevance_score=confidence,
                        context={"source_content": consumed_content, "suggested_content": suggested_content},
                        model_version=self.model_version,
                        algorithm="content_based",
                    ))
        ...
        if session_times:
            ...
            if evening_usage / len(session_times) > 0.4:
                recommendations.append(RecommendationCreate(
                    ...
                    priority_score=0.3,
                    relevance_score=0.7,
                    context={"evening_usage_ratio": evening_usage / len(session_times)},
                    model_version=self.model_version,
                    algorithm="content_based",
                ))
    ...
    async def _get_similar_users(self, user_id: int, limit: int = 10) -> List[SimilarUsers]:
    ...
    async def _get_user_events_analysis(self, user_ids: List[int]) -> Dict[int, Dict[str, Any]]:
        ...
        for user_id in user_ids:
            ...
            analysis = {
                "features_used": list(set(e.event_name for e in user_events if e.event_type == "interaction" and "feature_" in e.event_name)),
    ...
    def _extract_session_patterns(self, events: List[Event]) -> Dict[str, Any]:
        ...
        avg_session_length = np.mean([len(session) for session in sessions.values()]) if sessions else 0
        
        return {
            "avg_session_length": avg_session_length,
            "total_sessions": len(sessions),
            "session_frequency": len(sessions) / 30 if sessions else 0,  # sessions per day over 30 days
        }
    ...
    def _extract_usage_patterns(self, events: List[Event]) -> Dict[str, Any]:
        ...
        frequent_actions = sorted(action_counts.items(), key=lambda x: x[1], reverse=True)[:5]
    ...
    def _extract_incomplete_workflows(self, events: List[Event]) -> List[str]:
        ...
        workflows = {
            ...
            "project_creation": ["create_project", "add_team_members", "configure_settings"],
    ...
    def _extract_reading_patterns(self, events: List[Event]) -> Dict[str, Any]:
        ...
        reading_events = [e for e in events if e.event_type == "interaction" and "read" in e.event_name]
        
        if not reading_events:
            return {}
            
        return {
            "avg_reading_session": len(reading_events) / len(set(e.session_id for e in reading_events if e.session_id)),
...
class SimilarityEngine:
    ...
    async def _compute_similarities_for_user(
        ...
        for other_user_id in other_user_ids:
            ...
            similarity = self._calculate_similarity(user_vector, other_vector, algorithm)
            
            if similarity > 0.1:  # Only store significant similarities
                similarities.append(SimilarUsersCreate(
                    user_id=user_id,
        ...
        if similarities:
            ...
            await similar_users.create_bulk_similarities(self.db, similarities=similarities)
