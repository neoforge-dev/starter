...
class RuleEvaluator:
    ...
    async def apply_rule(
        ...
        try:
            ...
            if rule.personalization_type == "ui_adaptation":
                ...
                return await self._apply_workflow_optimization(db, rule, profile, base_config)
            elif rule.personalization_type == "navigation_customization":
                return await self._apply_navigation_customization(db, rule, profile, base_config)
            else:
                logger.warning(f"Unknown personalization type: {rule.personalization_type}")
                return {}
    ...
    async def _evaluate_conditions(
        ...
        for condition_type, condition_value in conditions.items():
            ...
            if condition_type == "min_sessions":
                ...
                if not profile.predicted_churn_risk or profile.predicted_churn_risk < condition_value:
                    return False
                    
            elif condition_type == "max_churn_risk":
                if profile.predicted_churn_risk and profile.predicted_churn_risk > condition_value:
                    return False
    ...
    async def _apply_content_filter(
        ...
        if "content_types" in config:
            ...
            preferred_types = profile.content_preferences.get("preferred_types", []) if profile.content_preferences else []
            allowed_types = list(set(config["content_types"]) & set(preferred_types)) if preferred_types else config["content_types"]
        ...
        if "personalized_recommendations" in config:
            content_filters["show_recommendations"] = config["personalized_recommendations"]
    ...
    async def _apply_navigation_customization(
        ...
        if "menu_order" in config:
            ...
            if most_used:
                ...
                menu_order = most_used + [item for item in config["menu_order"] if item not in most_used]
        ...
        if "quick_actions" in config:
            ...
            relevant_actions = [action for action in config["quick_actions"] if action in user_actions]
            navigation_customizations["quick_actions"] = relevant_actions or config["quick_actions"][:3]
        ...
        if "sidebar_collapsed" in config:
            ...
            if device_prefs.get("prefers_full_width", False):
                ...
                navigation_customizations["sidebar_collapsed"] = config["sidebar_collapsed"]
