...
class UserProfiler:
    ...
    async def compute_full_profile(
        ...
        try:
            ...
            logger.info(
                f"Computed profile for user {user_id}: segment={profile_data.get('primary_segment')}"
            )
            return profile_data
    ...
    async def update_profile_incrementally(
        self, profile: PersonalizationProfile, interaction_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if interaction_type == "feature_used":
                feature_usage = (
                    profile.feature_usage.copy() if profile.feature_usage else {}
                )
            ...
            if context and interaction_type in ["clicked", "viewed"]:
                nav_patterns = (
                    profile.navigation_patterns.copy()
                    if profile.navigation_patterns
                    else {}
                )
            ...
            if outcome == "positive":
                usage_patterns = (
                    profile.usage_patterns.copy() if profile.usage_patterns else {}
                )
                usage_patterns["positive_interactions"] = (
                    usage_patterns.get("positive_interactions", 0) + 1
                )
                usage_patterns[
            ...
            if updated_data:
                ...
                total_features = updated_data.get(
            ...
            logger.error(
                f"Error updating profile incrementally for user {profile.user_id}: {e}"
            )
            return {}

    async def _analyze_sessions(self, db: AsyncSession, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await db.execute(
                select(
                    ...
                    func.avg(
                        func.extract(
                            "epoch", UserSession.ended_at - UserSession.started_at
                        )
                        / 60
                    ...
                    and_(
                        UserSession.user_id == user_id, UserSession.ended_at.isnot(None)
                    )
                )
            )
            stats = result.first()
            ...
            session_data = {
                ...
                "avg_session_duration": float(stats.avg_duration_minutes)
            ...
            if stats.total_sessions and stats.total_sessions > 0:
                ...
                session_data["usage_patterns"] = await self._analyze_session_patterns(
                    sessions
                )
    ...
    async def _analyze_events(self, db: AsyncSession, user_id: int) -> Dict[str, Any]:
    ...
    async def _determine_user_segment(
        ...
        try:
            ...
            # Power user
                ...
                total_sessions > 50 and features_adopted > 15
            ) or avg_session_duration > 45:
                segment = "power_user"
                confidence = 0.9
            ...
            return {"primary_segment": "new_user", "segment_confidence": 0.5}
    ...
    async def _analyze_device_usage(self, events: List[Event]) -> Dict[str, Any]:
        ...
        return {
            ...
            "device_distribution": {
                device: count / total_events for device, count in device_counts.items()
            },
            "multi_device_user": len(device_counts) > 1,
        }

    async def _analyze_content_engagement(self, events: List[Event]) -> Dict[str, Any]:
