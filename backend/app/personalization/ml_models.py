...
from app.models.personalization import PersonalizationProfile, PersonalizationRule, PersonalizationInteraction
...
class PersonalizationMLModels:
    ...
    async def generate_user_insights(
        ...
        try:
            ...
            if churn_risk is not None:
                ...
                insights["churn_factors"] = await self.churn_model.get_risk_factors(db, profile)
            ...
            recommendations = await self._generate_personalization_recommendations(db, profile)
            if recommendations:
            ...
            logger.error(f"Error generating user insights for user {profile.user_id}: {e}")
            return None
    ...
    async def generate_personalized_ui(
        ...
        try:
            ...
            logger.error(f"Error generating personalized UI for user {profile.user_id}: {e}")
            return {}
            ...
            for segment, metrics in segment_analysis.items():
                if metrics["effectiveness"] < 0.6:  # Below 60% effectiveness
                    recommendations.append({
                        ...
                        "suggested_actions": await self._suggest_segment_improvements(db, segment, metrics)
                    })
            ...
            for context, performance in context_analysis.items():
                if performance["success_rate"] < 0.5:
                    recommendations.append({
                        ...
                        "suggested_actions": await self._suggest_context_improvements(db, context, performance)
                    })
    ...
    async def _predict_next_actions(
        ...
        try:
            ...
            if segment == "new_user":
                ...
                next_actions = ["master_current_features", "provide_feedback", "try_integrations"]
            elif segment == "power_user":
                next_actions = ["optimize_workflow", "use_advanced_features", "mentor_others"]
            elif segment == "goal_oriented":
                next_actions = ["complete_current_project", "set_new_goals", "track_progress"]
            ...
            if unused_features:
                next_actions.extend([f"try_{feature}" for feature in unused_features[:2]])
            
            return next_actions[:5]  # Return top 5 predictions
            
        except Exception as e:
            logger.error(f"Error predicting next actions for user {profile.user_id}: {e}")
            return []
    ...
    async def _find_engagement_opportunities(
        ...
        try:
            ...
            if profile.last_active_days > 7:
                opportunities.append({
                    "type": "reactivation",
                    "priority": "high" if profile.last_active_days > 30 else "medium",
            ...
            logger.error(f"Error finding engagement opportunities for user {profile.user_id}: {e}")
            return []
        ...
        try:
            ...
            if most_used_features:
                recommendations.append({
                    ...
                    "implementation": {
                        "featured_content": [feature for feature, _ in most_used_features],
            ...
            logger.error(f"Error generating personalization recommendations for user {profile.user_id}: {e}")
            return []
    ...
    async def _predict_layout_preferences(
        ...
        try:
            ...
            if context == "dashboard":
                preferences["widget_priority"] = await self._predict_widget_priorities(db, profile)
            ...
            logger.error(f"Error predicting layout preferences for user {profile.user_id}: {e}")
            return {}
    ...
    async def _predict_content_priorities(
        ...
        try:
            ...
            if feature_usage:
                ...
                priorities["feature_related_content"] = [feature for feature, _ in top_features]
            ...
            logger.error(f"Error predicting content priorities for user {profile.user_id}: {e}")
            return {}
    ...
    async def _predict_feature_visibility(
        ...
        try:
            ...
            advanced_features = ["analytics", "integrations", "automation", "api_access"]
            ...
            logger.error(f"Error predicting feature visibility for user {profile.user_id}: {e}")
            return {}
    ...
    def _get_unused_features(self, feature_usage: Dict[str, int]) -> List[str]:
        ...
        all_features = [
            ...
            "collaboration", "notifications", "search", "filters", "exports"
        ]
    ...
    async def _predict_widget_priorities(
        ...
        try:
            ...
            logger.error(f"Error predicting widget priorities for user {profile.user_id}: {e}")
            return ["overview"]
        ...
        try:
            ...
            if not segments:
                result = await db.execute(
                    select(PersonalizationProfile.primary_segment)
                    .distinct()
                )
