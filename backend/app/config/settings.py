...
class Settings(BaseSettings):
    ...
    ip_block_duration: int = Field(
        default=3600, env="IP_BLOCK_DURATION"
    )  # 1 hour in seconds
    threat_threshold: int = Field(
        default=3, env="THREAT_THRESHOLD"
    )  # Block IP after N threats
    threat_window: int = Field(
        default=3600, env="THREAT_WINDOW"
    )  # Time window in seconds for threat counting
    enable_request_fingerprinting: bool = Field(
        default=True, env="ENABLE_REQUEST_FINGERPRINTING"
    )
    ...
    account_lockout_duration: int = Field(
        default=1800, env="ACCOUNT_LOCKOUT_DURATION"
    )  # 30 minutes
    smtp_user: Optional[str] = Field(default=None, env="SMTP_USER")
    smtp_password: Optional[str] = Field(default=None, env="SMTP_PASSWORD")
    # Webhook settings
    SENDGRID_WEBHOOK_PUBLIC_KEY: Optional[str] = Field(
        default=None, env="SENDGRID_WEBHOOK_PUBLIC_KEY"
    )
    ...
    otel_exporter_otlp_traces_endpoint: Optional[str] = Field(
        default=None, env="OTEL_EXPORTER_OTLP_TRACES_ENDPOINT"
    )
    ...
    otel_exporter_otlp_traces_headers: Optional[str] = Field(
        default=None, env="OTEL_EXPORTER_OTLP_TRACES_HEADERS"
    )
    otel_exporter_otlp_protocol: str = Field(
        default="http/protobuf", env="OTEL_EXPORTER_OTLP_PROTOCOL"
    )
    otel_exporter_otlp_traces_protocol: Optional[str] = Field(
        default=None, env="OTEL_EXPORTER_OTLP_TRACES_PROTOCOL"
    )
    otel_exporter_otlp_timeout: int = Field(
        default=10, env="OTEL_EXPORTER_OTLP_TIMEOUT"
    )
    otel_exporter_otlp_traces_timeout: Optional[int] = Field(
        default=None, env="OTEL_EXPORTER_OTLP_TRACES_TIMEOUT"
    )
    otel_exporter_otlp_compression: Optional[str] = Field(
        default=None, env="OTEL_EXPORTER_OTLP_COMPRESSION"
    )
    otel_exporter_otlp_traces_compression: Optional[str] = Field(
        default=None, env="OTEL_EXPORTER_OTLP_TRACES_COMPRESSION"
    )

    # Service configuration
    otel_service_name: str = Field(default="neoforge-api", env="OTEL_SERVICE_NAME")
    otel_service_version: Optional[str] = Field(
        default=None, env="OTEL_SERVICE_VERSION"
    )
    otel_resource_attributes: Optional[str] = Field(
        default=None, env="OTEL_RESOURCE_ATTRIBUTES"
    )

    # Trace configuration
    otel_traces_exporter: str = Field(
        default="console", env="OTEL_TRACES_EXPORTER"
    )  # console, otlp, none
    otel_traces_sampler: str = Field(
        default="always_on", env="OTEL_TRACES_SAMPLER"
    )  # always_on, always_off, ratio
    otel_traces_sampler_arg: Optional[str] = Field(
        default=None, env="OTEL_TRACES_SAMPLER_ARG"
    )
    ...
    stripe_webhook_tolerance: int = Field(default=300, env="STRIPE_WEBHOOK_TOLERANCE")  # 5 minutes

    # Billing Configuration
    default_currency: str = Field(default="USD", env="DEFAULT_CURRENCY")
    trial_days_default: int = Field(default=14, env="TRIAL_DAYS_DEFAULT")
    proration_enabled: bool = Field(default=True, env="PRORATION_ENABLED")
    ...
    def validate_environment(cls, v: str) -> Environment:
        ...
        try:
            ...
            if v_lower not in [e.value for e in Environment]:
                raise ValueError(
                    f"Invalid environment: {v}. Must be one of {[e.value for e in Environment]}"
                )
    ...
    def validate_cors_origins(
        ...
        testing = str(info.data.get("testing", "false")).lower() in (
            "true",
            "1",
            "t",
            "yes",
        ...
        if isinstance(v, str):
            try:
                ...
                raise ValueError(
                    "CORS_ORIGINS must be a valid JSON string if provided as a string"
                )
        ...
        for origin in v:
            try:
                ...
                Exception
            ) as e:  # Catch Pydantic validation error more specifically if possible
    ...
    def adjust_database_url_for_environment(
    ...
    def validate_otel_traces_exporter(cls, v: str) -> str:
        ...
        if v not in valid_exporters:
            raise ValueError(
                f"OTEL_TRACES_EXPORTER must be one of: {', '.join(valid_exporters)}"
            )
        return v
    ...
    def validate_otel_traces_sampler(cls, v: str) -> str:
        ...
        if v not in valid_samplers:
            raise ValueError(
                f"OTEL_TRACES_SAMPLER must be one of: {', '.join(valid_samplers)}"
            )
        return v
    ...
    def validate_otel_protocol(cls, v: str) -> str:
        ...
        if v not in valid_protocols:
            raise ValueError(
                f"OTEL_EXPORTER_OTLP_PROTOCOL must be one of: {', '.join(valid_protocols)}"
            )
        return v
