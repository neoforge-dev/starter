...
from app.models.ab_test import AbTest, AbTestAssignment, AbTestVariant
...
class CRUDAbTest(CRUDBase[AbTest, AbTestCreate, AbTestUpdate]):
    ...
    async def get_by_test_key(
        self, db: AsyncSession, *, test_key: str
    ) -> Optional[AbTest]:
    ...
    async def get_test_analytics(
        ...
        *,
        ...
        for stats in variant_stats:
            p_value, ci = self._calculate_significance(
                control_variant, stats, test.confidence_level
            )
        ...
        overall_p_value = min(
            (s.p_value for s in variant_stats if s.p_value), default=None
        )
        ...
        return AbTestStatisticalReport(
            ...
            total_participants=sum(
                len([a for a in assignments if a.variant_id == v.id])
    ...
    async def get_test_performance_metrics(
        ...
        *,
        ...
        metrics = {
            ...
            "completed_tests": len(
                [t for t in tests if t.status == AbTestStatus.COMPLETED]
            ),
            "significant_tests": len(
                [t for t in tests if t.is_statistically_significant]
            ),
        ...
        if tests:
            ...
            participants_query = select(func.count(AbTestAssignment.id)).where(
                AbTestAssignment.test_id.in_(test_ids_list)
            )
            ...
            if eligible_for_completion > 0:
                metrics["test_completion_rate"] = (
                    metrics["completed_tests"] / eligible_for_completion
                )
    ...
    def _should_include_in_test(
        self, user_hash: str, traffic_allocation: float
    ) -> bool:
    ...
    def _assign_to_variant(
        self, variants: List[AbTestVariant], user_hash: str
    ...
    async def _update_variant_metrics(self, db: AsyncSession, variant_id: int) -> None:
        ...
        assignments_query = select(
            ...
            func.sum(func.cast(AbTestAssignment.converted, int)).label(
                "total_conversions"
            ),
    ...
    def _generate_insights(
        ...
        if insufficient_sample:
            insights.append(
                f"Sample size below minimum threshold ({min_sample} per variant)"
            )
        ...
        if is_significant:
            best_treatment = max(
                treatments, key=lambda x: x.conversion_rate, default=None
            )

            if (
                best_treatment
                and best_treatment.conversion_rate > control.conversion_rate
            ):
                improvement = best_treatment.relative_improvement or 0
                insights.append(
                    f"Variant {best_treatment.variant_key} shows {improvement:.1%} improvement"
                )
            ...
            if best_treatment:
                ...
                if improvement >= test.minimum_detectable_effect:
                    insights.append(
                        f"Promising trend in {best_treatment.variant_key} (+{improvement:.1%})"
                    )
