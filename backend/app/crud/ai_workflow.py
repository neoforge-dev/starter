...
class CRUDWorkflowSession(CRUDBase[WorkflowSession, WorkflowSessionCreate, WorkflowSessionUpdate]):
...
class CRUDWorkflowCheckpoint(CRUDBase[WorkflowCheckpoint, WorkflowCheckpointCreate, WorkflowCheckpointUpdate]):
    ...
    async def get_by_checkpoint_id(
        ...
        result = await db.execute(
            select(WorkflowCheckpoint).where(WorkflowCheckpoint.checkpoint_id == checkpoint_id)
        )
        return result.scalar_one_or_none()
    ...
    async def get_session_checkpoints(
        ...
        query = select(WorkflowCheckpoint).where(WorkflowCheckpoint.session_id == session_id)
...
class CRUDAgentMessage(CRUDBase[AgentMessage, AgentMessageCreate, AgentMessageUpdate]):
    ...
    async def get_agent_messages(
        ...
        query = (
            query
            .order_by(asc(AgentMessage.priority), desc(AgentMessage.created_at))
            .limit(limit)
        )
    ...
    async def get_ready_tasks(
        ...
        query = (
            ...
            .where(
                and_(
                    Task.status == "pending",
                    ~Task.id.in_(unmet_deps_subquery)
                )
            )
        )
...
class CRUDQualityGateExecution(CRUDBase[QualityGateExecution, QualityGateExecutionCreate, QualityGateExecutionUpdate]):
    ...
    async def get_by_execution_id(
        ...
        result = await db.execute(
            select(QualityGateExecution).where(QualityGateExecution.execution_id == execution_id)
        )
        return result.scalar_one_or_none()
    ...
    async def get_gate_statistics(
        ...
        query = select(QualityGateExecution).where(QualityGateExecution.started_at >= start_time)
...
class CRUDAgentRegistry(CRUDBase[AgentRegistry, AgentRegistryCreate, AgentRegistryUpdate]):
