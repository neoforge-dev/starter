...
from app.core.tenant import TenantIsolationError, PermissionDeniedError
...
class TenantCRUDBase(CRUDBase[ModelType, CreateSchemaType, UpdateSchemaType], ABC):
    ...
    async def create_with_tenant(
        ...
        *,
        ...
        await self._log_operation(
            db, "create", db_obj.id, tenant_id, created_by
        )
        
        logger.debug(
            f"Created {self.model.__name__} {db_obj.id} in tenant {tenant_id}"
        )
    ...
    async def update_with_tenant(
        ...
        *,
        ...
        # Log update
        await self._log_operation(
            db, "update", db_obj.id, tenant_id, updated_by
        )
        
        logger.debug(
            f"Updated {self.model.__name__} {db_obj.id} in tenant {tenant_id}"
        )
    ...
    async def remove_with_tenant(
        ...
        *,
        ...
        # Log deletion
        await self._log_operation(
            db, "delete", obj.id, tenant_id, deleted_by
        )
        
        logger.debug(
            f"Deleted {self.model.__name__} {obj.id} from tenant {tenant_id}"
        )
    ...
    async def _check_resource_permissions(
        ...
        ).join(Permission).where(Permission.name == permission_name)
...
class OrganizationScopedCRUD(TenantCRUDBase[ModelType, CreateSchemaType, UpdateSchemaType]):
    """
    ...
    async def get_multi_with_hierarchy(
        ...
        *,
        ...
        if self.organization_id_field:
            query = query.where(
                getattr(self.model, 'organization_id').in_(org_ids)
            )
    ...
    async def _get_child_organizations(
        ...
        cte_recursive = cte.union_all(
            select(
                ...
                Organization.join(cte, Organization.parent_id == cte.c.id)
