...
class CRUDPasswordResetToken(CRUDBase[PasswordResetToken, None, None]):
    ...
    async def mark_as_used(self, db: AsyncSession, token: PasswordResetToken) -> None:
    ...
    async def cleanup_tokens_for_user(self, db: AsyncSession, user_id: int) -> int:
    ...
    async def has_recent_token(
        self, db: AsyncSession, user_id: int, minutes_threshold: int = 5
    ) -> bool:
