...
from sqlalchemy import and_, desc, func, or_, select, text, update, case
...
class CRUDPersonalizationProfile(CRUDBase[PersonalizationProfile, PersonalizationProfileCreate, PersonalizationProfileUpdate]):
...
class CRUDPersonalizationRule(CRUDBase[PersonalizationRule, PersonalizationRuleCreate, PersonalizationRuleUpdate]):
    ...
    async def create_rule(
        ...
        db: AsyncSession,
        *,
        obj_in: PersonalizationRuleCreate
    ) -> PersonalizationRule:
        ...
        *,
        ...
        if context:
            context_condition = func.jsonb_exists(
                PersonalizationRule.target_contexts,
                context
            )
    ...
    async def get_rule_performance_stats(
        ...
        *,
        ...
        result = await db.execute(
            select(
                func.count(PersonalizationInteraction.interaction_id).label("total_interactions"),
                func.count(
                    case(
                ...
                func.avg(PersonalizationInteraction.response_time_ms).label("avg_response_time"),
                func.avg(PersonalizationInteraction.engagement_score).label("avg_engagement")
            )
            .where(
                and_(
        ...
        return {
            ...
            "success_rate": (stats.positive_outcomes / stats.total_interactions) if stats.total_interactions else 0,
            "avg_response_time_ms": float(stats.avg_response_time) if stats.avg_response_time else 0,
            "avg_engagement_score": float(stats.avg_engagement) if stats.avg_engagement else 0,
            "period_days": days
        }


class CRUDPersonalizationInteraction(CRUDBase[PersonalizationInteraction, PersonalizationInteractionCreate, None]):
    ...
    async def create_interaction(
        ...
        db: AsyncSession,
        *,
        obj_in: PersonalizationInteractionCreate
    ) -> PersonalizationInteraction:
    ...
    async def create_bulk_interactions(
        ...
        db: AsyncSession,
        *,
        interactions: List[PersonalizationInteractionCreate]
        ...
        *,
        ...
        result = await db.execute(
            select(
                ...
                func.avg(PersonalizationInteraction.response_time_ms).label("avg_response_time"),
                func.avg(PersonalizationInteraction.engagement_score).label("avg_engagement"),
                func.count(func.distinct(PersonalizationInteraction.user_id)).label("unique_users")
            )
            .where(and_(*conditions))
        )
        stats = result.first()
        ...
        return {
            ...
            "positive_outcomes": stats.positive_outcomes or 0,
            "click_through_rate": (stats.clicks / stats.total_interactions) if stats.total_interactions else 0,
            "success_rate": (stats.positive_outcomes / stats.total_interactions) if stats.total_interactions else 0,
            "avg_response_time_ms": float(stats.avg_response_time) if stats.avg_response_time else 0,
            "avg_engagement_score": float(stats.avg_engagement) if stats.avg_engagement else 0,
...
class CRUDPersonalizationAnalytics:
        ...
        *,
        ...
        result = await db.execute(
            select(
                ...
                func.count(func.distinct(PersonalizationProfile.user_id)).label("total_users"),
                func.avg(PersonalizationProfile.avg_session_duration).label("avg_session_duration"),
                func.avg(PersonalizationProfile.predicted_churn_risk).label("avg_churn_risk"),
                func.avg(PersonalizationProfile.lifetime_value_score).label("avg_lifetime_value")
            )
            .join(
                PersonalizationInteraction,
        ...
        for row in result:
            segment_data[row.primary_segment] = {
                ...
                "avg_churn_risk": float(row.avg_churn_risk) if row.avg_churn_risk else 0,
                "avg_lifetime_value": float(row.avg_lifetime_value) if row.avg_lifetime_value else 0
            }
    ...
    async def get_top_performing_rules(
        ...
        *,
        ...
        rules = []
        for row in result:
            success_rate = (row.positive_outcomes / row.total_interactions) if row.total_interactions else 0
            rules.append({
                "rule_id": row.rule_id,
                "name": row.name,
    ...
    async def compute_segment_analysis(
        ...
        *,
        ...
        interaction_result = await db.execute(
            select(
                func.count(func.distinct(PersonalizationInteraction.user_id)).label("active_users"),
                func.count(
                    case(
                        (PersonalizationInteraction.outcome == "positive", 1),
                        else_=None
        ...
        conversion_rate = (
            ...
            if interaction_stats.total_interactions else 0
        )
        ...
        analysis = PersonalizationSegmentAnalysis(
            ...
            avg_session_duration=float(profile_stats.avg_session_duration) if profile_stats.avg_session_duration else None,
