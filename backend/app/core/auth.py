...
async def validate_refresh_token(redis: Redis, token: str) -> Optional[dict]:
    """
...
async def revoke_refresh_token(
    redis: Redis, token: str, user_id: Optional[int] = None
) -> bool:
    """
    ...
    if deleted:
        logger.info(
            "refresh_token_revoked", user_id=user_id, token_hash=token_hash[:8] + "..."
        )
...
async def revoke_all_user_tokens(redis: Redis, user_id: int) -> int:
    ...
    logger.info("all_user_tokens_revoked", user_id=user_id, revoked_count=revoked_count)
