...
def setup_otlp_tracer_provider(settings, service_name_suffix: str = "") -> Optional[Any]:
    ...
    try:
        ...
        if (settings.otel_sdk_disabled or 
            settings.otel_traces_exporter == "none" or
            settings.environment == "test"):
            logger.info("otel_tracing_disabled", 
                       reason="sdk_disabled" if settings.otel_sdk_disabled else "exporter_none")
            return None
        ...
        from opentelemetry.sdk.trace.sampling import (
            AlwaysOnSampler, AlwaysOffSampler, TraceIdRatioBasedSampler
        )
        ...
        if settings.otel_traces_sampler == "always_off":
            ...
            if settings.otel_traces_sampler_arg:
                try:
                    ...
                    logger.warning("otel_invalid_sampler_ratio", 
        ...
        logger.info("otel_tracer_provider_configured",
        ...
        logger.warning("otel_dependencies_missing", error=str(e),
...
def _create_span_processor(settings) -> Optional[Any]:
    ...
    try:
        ...
        if settings.otel_traces_exporter == "console":
            ...
            logger.warning("otel_unknown_exporter", exporter=settings.otel_traces_exporter)
            return None
...
def _create_otlp_processor(settings) -> Optional[Any]:
    ...
    try:
        ...
        if not endpoint:
            logger.warning("otel_otlp_endpoint_missing", 
        ...
        protocol = (settings.otel_exporter_otlp_traces_protocol or 
                   settings.otel_exporter_otlp_protocol)

        # Parse headers - traces-specific takes precedence
        headers_str = (settings.otel_exporter_otlp_traces_headers or 
        ...
        timeout = (settings.otel_exporter_otlp_traces_timeout or 
                  settings.otel_exporter_otlp_timeout)

        # Determine compression - traces-specific takes precedence  
        compression = (settings.otel_exporter_otlp_traces_compression or
                      settings.otel_exporter_otlp_compression)
...
def _create_http_otlp_processor(endpoint: str, headers: Optional[Dict], 
                               timeout: int, compression: Optional[str]) -> Optional[Any]:
    ...
    try:
        ...
        logger.info("otel_http_otlp_exporter_configured",
                   endpoint=endpoint,
                   timeout=timeout,
        ...
        logger.warning("otel_http_otlp_exporter_unavailable",
                      note="Install opentelemetry-exporter-otlp for HTTP OTLP support")
        return None


def _create_grpc_otlp_processor(endpoint: str, headers: Optional[Dict],
                               timeout: int, compression: Optional[str]) -> Optional[Any]:
    ...
    try:
        ...
        logger.info("otel_grpc_otlp_exporter_configured",
                   endpoint=endpoint,
                   timeout=timeout,
                   compression=compression,
                   headers_count=len(headers) if headers else 0)
        ...
        logger.warning("otel_grpc_otlp_exporter_unavailable",
