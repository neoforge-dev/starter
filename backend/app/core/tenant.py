...
from app.models.rbac import Permission, PermissionCache, ResourcePermission, Role
...
class TenantManager:
    ...
    async def suspend_tenant(
        self, tenant_id: int, reason: str, suspended_by: Optional[int] = None
    ) -> Tenant:
        """Suspend a tenant."""
        tenant = await self.db.get(Tenant, tenant_id)
        if not tenant:
...
class PermissionManager:
    ...
    async def check_permission(
        ...
        """
        # Try cache first
        cache_key = self._get_permission_cache_key(user_id, tenant_id, organization_id)
    ...
    async def assign_role(
        ...
        try:
            ...
            if role.tenant_id != tenant_id:
                raise ValueError(
                    f"Role {role_id} does not belong to tenant {tenant_id}"
                )

            if organization_id and role.organization_id != organization_id:
                raise ValueError(
                    f"Role {role_id} does not belong to organization {organization_id}"
                )
            ...
            await self._invalidate_user_permission_cache(
    ...
    def _get_permission_cache_key(
        self, user_id: int, tenant_id: int, organization_id: Optional[int] = None
    ) -> str:
        """Generate cache key for user permissions."""
        if organization_id:
            return (
                f"tenant:{tenant_id}:permissions:user:{user_id}:org:{organization_id}"
            )
