...
class TemplateSchema(BaseModel):
    ...
    required_params: List[str] = Field(
        default_factory=list, description="Required template parameters"
    )
    optional_params: List[str] = Field(
        default_factory=list, description="Optional template parameters"
    )
    example_data: Dict[str, Any] = Field(
        default_factory=dict, description="Example data for testing"
    )
    ...
    def check_no_overlap(cls, values):
        ...
        if overlap:
            raise ValueError(
                f"Optional parameters overlap with required parameters: {overlap}"
            )
        return values
...
class TemplateValidator:
    ...
    def __init__(self, template_dir: Optional[Path] = None):
        ...
        self.template_dir = (
            template_dir or Path(__file__).parent.parent / "email_templates"
        )
    ...
    def get_template_variables(self, template_name: str) -> set[str]:
        ...
        try:
            ...
            template_source = self.env.loader.get_source(self.env, template_full_path)[
                0
            ]
            # Parse template
    ...
    def validate_template_data(self, template_name: str, data: Dict[str, Any]) -> None:
        ...
        missing_params = [
            param for param in schema.required_params if param not in data
        ]
        if missing_params:
            raise TemplateError(
    ...
    def render_template(self, template_name: str, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Use the full path in the error message
            raise ValueError(
                f"Error rendering template email/{template_name}.html: {e}"
            )
