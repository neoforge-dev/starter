...
class QueryPerformanceTracker:
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        sorted_by_count = sorted(self.query_counts.items(), key=lambda x: x[1], reverse=True)
        report['top_queries_by_count'] = [
            {'query': query, 'count': count} 
            for query, count in sorted_by_count[:10]
        ]
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if max_count > 100:
            recommendations.append(
                "Consider caching results for most frequent queries"
            )
        
        # Check for N+1 query patterns
        select_counts = sum(1 for query in self.query_counts.keys() if 'select' in query)
...
async def analyze_query_performance(engine: AsyncEngine, query: str) -> Dict[str, Any]:
    ...
    async with engine.begin() as conn:
        ...
        if 'Seq Scan' in plan.get('Node Type', ''):
            analysis['recommendations'].append("Consider adding an index to avoid sequential scan")
        
        # Check for high cost operations
        if plan.get('Total Cost', 0) > 1000:
            analysis['recommendations'].append("High cost operation detected - consider query optimization")
        
        # Check for large row counts
        if plan.get('Actual Rows', 0) > 10000:
            analysis['recommendations'].append("Large result set - consider adding filters or pagination")
...
async def get_database_stats(engine: AsyncEngine) -> Dict[str, Any]:
    ...
    async with engine.begin() as conn:
        ...
        # Connection stats
        connection_stats_result = await conn.execute(text("""
            SELECT 
...
async def vacuum_analyze_tables(engine: AsyncEngine, tables: Optional[List[str]] = None) -> None:
