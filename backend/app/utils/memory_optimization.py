...
class MemoryMonitor:
    ...
    def _check_memory_alerts(self, metrics: MemoryMetrics):
        ...
        for alert_name, alert in self.alerts.items():
            if alert.should_alert(
                metrics.process_memory_mb, metrics.process_memory_percent
            ):
                alert.trigger_alert()
    ...
    def get_memory_report(self) -> Dict[str, Any]:
        ...
        recent_metrics = [
            m for m in self.metrics_history[-60:] if m.timestamp > hour_ago
        ]
        ...
        if recent_metrics:
            ...
            avg_objects = sum(m.heap_objects for m in recent_metrics) / len(
                recent_metrics
            )
        else:
        ...
        return {
            ...
            "alerts": {
                name: {
                    "alert_count": alert.alert_count,
                    "last_alert": alert.last_alert_time.isoformat()
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if current.process_memory_mb > 1000:
            recommendations.append(
                "Process memory usage is high (>1GB). Consider implementing memory optimization strategies."
            )

        # High object count
        if current.heap_objects > 100000:
            recommendations.append(
                "High number of heap objects detected. Check for object leaks or unnecessary object creation."
            )

        # Frequent alerts
        if self.alerts["high_memory"].alert_count > 5:
            recommendations.append(
                "Frequent memory alerts detected. Consider increasing memory limits or optimizing memory usage."
            )
        ...
        if len(self.metrics_history) > 1:
            gc_rate = (
                recent_collections - self.metrics_history[-10].gc_collections
                if len(self.metrics_history) >= 10
                else 0
            )
            if gc_rate > 50:
                recommendations.append(
                    "High garbage collection frequency detected. Consider optimizing object creation patterns."
                )
...
class MemoryOptimizationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        response.headers[
            "X-Memory-Usage"
        ] = f"{current_metrics.process_memory_mb:.1f}MB"
        response.headers[
            "X-Memory-Percent"
        ] = f"{current_metrics.process_memory_percent:.1f}%"
