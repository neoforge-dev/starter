    ...
    try:
        ...
        if isinstance(payload, dict) and "data" in payload:
            ...
            data_hash = json.dumps(
                payload["data"], default=str, sort_keys=True, separators=(",", ":")
            )
        ...
        if context:
            context_hash = json.dumps(
                context, default=str, sort_keys=True, separators=(",", ":")
            )
...
def set_etag(
    response: Response, payload: Any, context: Optional[Dict[str, Any]] = None
) -> str:
...
def not_modified(request: Request, etag: str) -> bool:
    ...
    return any(
        etag == tag or etag.replace("W/", "") == tag.replace("W/", "") for tag in etags
    )
...
def compute_cache_key(
    endpoint: str, params: Dict[str, Any], user_id: Optional[int] = None
) -> str:
...
def is_cacheable_request(request: Request) -> bool:
    ...
    if request.headers.get("Authorization") and not request.headers.get(
        "X-Cache-Allowed"
    ):
        return False
