...
class PaginationPerformanceMonitor:
    ...
    def __init__(self):
        ...
        self.active_queries_gauge = Gauge(
            "pagination_active_queries", "Number of active pagination queries"
        )
    ...
    async def _record_metrics(self, metrics_data: PaginationMetrics):
        ...
        if metrics_data.cursor_size:
            self.cursor_size_histogram.labels(endpoint=metrics_data.endpoint).observe(
                metrics_data.cursor_size
            )
    ...
    def get_performance_summary(self, last_hours: int = 24) -> Dict[str, Any]:
        ...
        slow_queries = [
            m for m in recent_metrics if m.total_duration > self.slow_query_threshold
        ]
        ...
        p95_duration = (
            sorted([m.total_duration for m in recent_metrics])[
                int(total_queries * 0.95)
            ]
            if total_queries > 0
            else 0
        )
        ...
        return {
            "summary": {
                ...
                "cache_hit_rate": len(cache_hits) / total_queries
                if total_queries > 0
                else 0,
            ...
            "performance_by_type": {
                "cursor": {
                    "avg_duration": round(
                        sum(m.total_duration for m in cursor_queries)
                        / len(cursor_queries),
                        2,
                    )
                    if cursor_queries
                    ...
                    "slow_queries": len(
                        [
                            m
                            for m in cursor_queries
                            if m.total_duration > self.slow_query_threshold
                        ]
                    ),
                },
                "offset": {
                    "avg_duration": round(
                        sum(m.total_duration for m in offset_queries)
                        / len(offset_queries),
                        2,
                    )
                    if offset_queries
    ...
    def _get_endpoint_performance(
        ...
        for endpoint in set(m.endpoint for m in metrics):
            ...
            endpoint_stats[endpoint] = {
                ...
                "avg_duration": round(
                    sum(m.total_duration for m in endpoint_metrics)
                ...
                "cache_hit_rate": len([m for m in endpoint_metrics if m.cache_hit])
                / len(endpoint_metrics),
            }
    ...
    def _get_optimization_recommendations(
        ...
        cursor_rate = (
            len([m for m in metrics if m.pagination_type == "cursor"]) / len(metrics)
            if metrics
            else 0
        )
        if cursor_rate < 0.5:
            recommendations.append(
                "Consider migrating more queries to cursor-based pagination for better performance"
            )

        # Check cache hit rate
        cache_hit_rate = (
            len([m for m in metrics if m.cache_hit]) / len(metrics) if metrics else 0
        )
        ...
        # Check slow queries
        slow_queries = [
            m for m in metrics if m.total_duration > self.slow_query_threshold
        ]
        if len(slow_queries) > len(metrics) * 0.1:  # More than 10% slow queries
            recommendations.append(
                "Investigate slow queries - consider database index optimization"
            )
        ...
        if len(no_index_queries) > 0:
            recommendations.append(
                "Some queries are not using database indices - review query plans"
            )

        # Check response sizes
        large_responses = [
            m for m in metrics if m.response_size and m.response_size > 1024 * 1024
        ]  # 1MB
        if len(large_responses) > 0:
            recommendations.append(
                "Consider reducing page sizes for queries with large response payloads"
            )
