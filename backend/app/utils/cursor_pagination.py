...
class CursorData(BaseModel):
    ...
    last_value: Optional[Union[str, int, float]] = Field(None, description="Last value from previous page")
    last_id: Optional[int] = Field(None, description="Last ID from previous page for tie-breaking")
    filters: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Active filters")
...
class PaginationInfo(BaseModel):
    ...
    per_page: Optional[int] = Field(None, description="Backward compatibility: items per page")
...
class CursorPaginationManager:
    ...
    def build_cursor_query(
        ...
        if cursor_data and cursor_data.last_value is not None:
            ...
            if cursor_data.sort_by in ["created_at", "updated_at"] and isinstance(last_value, str):
    ...
    async def paginate(
        ...
        if reverse:
            ...
            has_next = cursor is not None  # If we reversed from a cursor, there's a next page
        ...
        if include_total:
            ...
            if cursor_data.filters or filters:
                ...
                count_base_query = self.apply_filters(count_base_query, model, combined_filters)
