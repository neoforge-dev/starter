...
from app.schemas.recommendation import RecommendationType, UserPreferencesUpdate
            ...
            for user_id in target_user_ids:
                try:
                    ...
                    for rec_create in new_recommendations:
                        await crud_recommendation.recommendation.create(
        ...
        async with AsyncSessionLocal() as db:
            ...
            if user_id:
                ...
                result = await db.execute(
                    ...
                    .where(Event.timestamp >= cutoff_date, Event.user_id.isnot(None))
                    .limit(500)
                )
            ...
            for uid in target_user_ids:
                try:
                    ...
                    preferences_analysis = await _analyze_user_events_for_preferences(
                        user_events
                    )
...
def generate_personalized_notifications_task(
            ...
            for user_id in target_user_ids:
                try:
                    ...
                    for rec_create in recommendations:
                        ...
                        rec_create.priority_score = min(
                            rec_create.priority_score + 0.2, 1.0
                        )  # Boost for notifications
                    ...
                    logger.error(
                        f"Error generating notifications for user {user_id}: {str(e)}"
                    )
                    errors += 1
                    continue
...
async def _analyze_user_events_for_preferences(events: List[Event]) -> dict:
    ...
    for event in events:
        ...
        if event.event_type == "interaction" and event.event_name.startswith(
            "feature_"
        ):
    ...
    evening_usage = sum(
        count for hour, count in time_of_day_usage.items() if 18 <= hour <= 23
    )
    ...
    behavioral_patterns = {
        ...
        "preferred_time_of_day": max(time_of_day_usage.items(), key=lambda x: x[1])[0]
        if time_of_day_usage
        else 12,
    ...
    weeks = (
        (max(unique_dates) - min(unique_dates)).days / 7 if len(unique_dates) > 1 else 1
    )
    weekly_active_days = (
        min(len(unique_dates) / weeks * 7, 7) if weeks > 0 else len(unique_dates)
    )
    ...
    feature_adoption_rate = min(
        len(unique_features) / 10, 1.0
