...
class ContentItem(Base):
    ...
    title: Mapped[str] = mapped_column(
        String(500), nullable=False, index=True, comment="Content title"
    )
    description: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Content description or summary"
    )
    content_type: Mapped[ContentType] = mapped_column(
        SQLEnum(ContentType), nullable=False, index=True, comment="Type of content"
    )
    category: Mapped[ContentCategory] = mapped_column(
        SQLEnum(ContentCategory), nullable=False, index=True, comment="Content category"
    )

    # Content metadata
    url: Mapped[Optional[str]] = mapped_column(
        String(2048), nullable=True, comment="Content URL if available"
    )
    author: Mapped[Optional[str]] = mapped_column(
        String(200), nullable=True, comment="Content author"
    )
    source: Mapped[Optional[str]] = mapped_column(
        String(200), nullable=True, comment="Content source or platform"
    )
    published_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, comment="When content was originally published"
    )

    # AI Analysis results
    quality_score: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, index=True, comment="AI-computed quality score (0.0-1.0)"
    )
    ...
    engagement_prediction: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Predicted engagement score"
    )
    sentiment_score: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Content sentiment analysis (-1.0 to 1.0)"
    )

    # Content analysis metadata
    topics: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="Extracted topics and keywords"
    )
    ai_analysis: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="Complete AI analysis results"
    )
    optimization_suggestions: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="AI-generated optimization suggestions"
    )

    # Engagement metrics
    view_count: Mapped[int] = mapped_column(
        Integer, default=0, comment="Number of views"
    )
    click_count: Mapped[int] = mapped_column(
        Integer, default=0, comment="Number of clicks"
    )
    share_count: Mapped[int] = mapped_column(
        Integer, default=0, comment="Number of shares"
    )
    engagement_rate: Mapped[float] = mapped_column(
        Float, default=0.0, comment="Overall engagement rate"
    )

    # Status and timing
    is_active: Mapped[bool] = mapped_column(
        Boolean, default=True, index=True, comment="Whether content is active"
    )
    ...
    analyzed_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, comment="When content was last analyzed by AI"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_content_items_quality_relevance", "quality_score", "relevance_score"
        ),
        ...
        # Performance indexes
        # Temporarily disabled GIN indexes due to PostgreSQL extension issues
...
class ContentSuggestion(Base):
    ...
    title: Mapped[str] = mapped_column(
        String(300), nullable=False, comment="Suggestion title/headline"
    )
    description: Mapped[str] = mapped_column(
        Text, nullable=False, comment="Suggestion description and reasoning"
    )
    call_to_action: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, comment="Call to action text"
    )
    ...
    personalization_score: Mapped[float] = mapped_column(
        Float, nullable=False, comment="How personalized this suggestion is"
    )

    # Suggestion context and metadata
    context: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="Context when suggestion was generated"
    )
    ai_reasoning: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="AI reasoning for this suggestion"
    )
    personalization_factors: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="Factors that influenced personalization"
    )
    ...
    impression_count: Mapped[int] = mapped_column(
        Integer, default=0, comment="Number of times shown to user"
    )
    click_count: Mapped[int] = mapped_column(
        Integer, default=0, comment="Number of clicks by user"
    )
    ...
    algorithm: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="Algorithm used for suggestion generation"
    )
    ...
    expires_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, index=True, comment="When suggestion expires"
    )
    first_shown_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, comment="When suggestion was first shown"
    )
    last_shown_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, comment="When suggestion was last shown"
    )
    clicked_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, comment="When user clicked suggestion"
    )
    dismissed_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, comment="When user dismissed suggestion"
    )

    # Relationships
    user: Mapped["User"] = relationship("User", lazy="selectin")
    content_item: Mapped["ContentItem"] = relationship(
        "ContentItem", back_populates="suggestions", lazy="selectin"
    )
    ...
    __table_args__ = (
        ...
        # JSONB indexing
        # Index("idx_content_suggestions_context_gin", "context", postgresql_using="gin"),  # Temporarily disabled
        # Index(
...
class ContentSuggestionFeedback(Base):
    ...
    session_data: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON, nullable=True, comment="User session data at time of feedback"
    )
    ...
    suggestion: Mapped["ContentSuggestion"] = relationship(
        "ContentSuggestion", back_populates="feedback_entries", lazy="selectin"
    )
    user: Mapped["User"] = relationship("User", lazy="selectin")
    ...
    __table_args__ = (
        ...
        # JSONB indexing
        # Index("idx_content_feedback_context_gin", "context", postgresql_using="gin"),  # Temporarily disabled
        # Index(
        #     "idx_content_feedback_session_gin", "session_data", postgresql_using="gin"  # Temporarily disabled
        # ),
    )
...
class ContentAnalysisJob(Base):
    ...
    progress: Mapped[float] = mapped_column(
        Float, default=0.0, comment="Job progress (0.0-1.0)"
    )

    # Analysis results
    results: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON, nullable=True, comment="Analysis results"
    )
    ...
    ai_model_used: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, comment="AI model used for analysis"
    )
    processing_time_seconds: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Time taken for processing"
    )
    tokens_used: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, comment="Number of tokens consumed"
    )
    cost_usd: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Cost of analysis in USD"
    )
    ...
    started_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, comment="When job started processing"
    )
    completed_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, comment="When job completed"
    )
    ...
    __table_args__ = (
        ...
        # JSONB indexing temporarily disabled
