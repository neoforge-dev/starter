...
from sqlalchemy import Boolean, DateTime, ForeignKey, Index, Integer, JSON, String, text, Float
...
class ConversionFunnel(Base):
    ...
    created_by_user_id: Mapped[Optional[int]] = mapped_column(
        Integer,
        nullable=True,
        comment="User who created this funnel configuration"
    )
    ...
    config: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON,
        nullable=True,
        comment="Additional funnel configuration options as JSON"
    )
    ...
    __table_args__ = (
        ...
        Index('idx_funnels_config_gin', 'config', postgresql_using='gin'),
    )
...
class FunnelStep(Base):
    ...
    required_properties: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON,
        nullable=True,
        comment="Required event properties for step completion"
    )
    property_filters: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON,
        nullable=True,
        comment="Advanced property filters (ranges, lists, etc.)"
    )
    ...
    max_time_from_previous_seconds: Mapped[Optional[int]] = mapped_column(
        Integer,
        nullable=True,
        comment="Maximum time allowed since previous step"
    )
    ...
    success_message: Mapped[Optional[str]] = mapped_column(
        String(200),
        nullable=True,
        comment="Message displayed when step is completed"
    )
    ...
    __table_args__ = (
        ...
        Index('idx_funnel_steps_properties_gin', 'required_properties', postgresql_using='gin'),
        Index('idx_funnel_steps_filters_gin', 'property_filters', postgresql_using='gin'),
...
class FunnelUserJourney(Base):
    ...
    total_steps_completed: Mapped[int] = mapped_column(
        Integer,
        default=0,
        comment="Total number of steps completed by the user"
    )
    total_value: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Total value generated through this journey"
    )
    ...
    journey_data: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON,
        nullable=True,
        comment="Additional journey-specific data and context"
    )
    ...
    __table_args__ = (
        ...
        Index('idx_journeys_data_gin', 'journey_data', postgresql_using='gin'),
    )
