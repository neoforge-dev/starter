...
class AbTest(Base):
    ...
    name: Mapped[str] = mapped_column(
        String(200), nullable=False, comment="Human-readable test name"
    )
    description: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Detailed test description and hypothesis"
    )
    ...
    start_date: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True, comment="Test start date and time"
    )
    end_date: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True, comment="Test end date and time"
    )
    ...
    created_by: Mapped[Optional[int]] = mapped_column(
        ForeignKey("users.id"), nullable=True, comment="User who created the test"
    )
    ...
    creator: Mapped[Optional["User"]] = relationship(
        "User", back_populates="created_ab_tests", lazy="selectin"
    )
...
class AbTestVariant(Base):
    ...
    name: Mapped[str] = mapped_column(
        String(200), nullable=False, comment="Human-readable variant name"
    )
    description: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Variant description and changes"
    )
    ...
    configuration: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON, nullable=True, comment="Variant-specific configuration and content"
    )
    ...
    confidence_interval_lower: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Lower bound of confidence interval"
    )
    confidence_interval_upper: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Upper bound of confidence interval"
    )
    p_value: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="P-value compared to control variant"
    )
...
class AbTestAssignment(Base):
    ...
    test_id: Mapped[int] = mapped_column(
        ForeignKey("ab_tests.id"), nullable=False, comment="Reference to A/B test"
    )
    ...
    session_id: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True, index=True, comment="Session ID for anonymous users"
    )
    ...
    converted_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True, comment="When conversion occurred"
    )
    conversion_value: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Monetary or numeric value of conversion"
    )

    # Additional tracking
    first_exposure_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True, comment="When user first saw the variant"
    )
    last_seen_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True, comment="Last time user interacted with test"
    )

    # Context and targeting
    context: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON, nullable=True, comment="Assignment context (user agent, location, etc.)"
    )

    # Relationships
    test: Mapped["AbTest"] = relationship(
        "AbTest", back_populates="assignments", lazy="selectin"
    )

    variant: Mapped["AbTestVariant"] = relationship(
        "AbTestVariant", back_populates="assignments", lazy="selectin"
    )

    user: Mapped[Optional["User"]] = relationship(
        "User", back_populates="ab_test_assignments", lazy="selectin"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_ab_test_assignments_conversion", "test_id", "converted", "converted_at"
        ),
