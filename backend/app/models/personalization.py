...
class PersonalizationProfile(Base):
    ...
    secondary_segments: Mapped[Optional[List[str]]] = mapped_column(
        JSONB, nullable=True, comment="Additional user segments"
    )
    ...
    feature_usage: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="Feature usage patterns and preferences"
    )
    navigation_patterns: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="Navigation preferences and common paths"
    )
    device_preferences: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="Device usage patterns and preferences"
    )

    # Engagement metrics
    total_sessions: Mapped[int] = mapped_column(
        Integer, default=0, comment="Total number of user sessions"
    )
    ...
    features_adopted: Mapped[int] = mapped_column(
        Integer, default=0, comment="Number of features user has adopted"
    )
    last_active_days: Mapped[int] = mapped_column(
        Integer, default=0, comment="Days since last activity"
    )

    # Personalization preferences
    ui_preferences: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="UI customization preferences"
    )
    content_preferences: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="Content type and format preferences"
    )
    notification_preferences: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="Notification timing and channel preferences"
    )

    # ML model insights
    predicted_churn_risk: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Predicted churn risk score (0.0-1.0)"
    )
    lifetime_value_score: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Predicted lifetime value score"
    )
    next_likely_actions: Mapped[Optional[List[str]]] = mapped_column(
        JSONB, nullable=True, comment="ML-predicted next likely user actions"
    )
    ...
    last_analyzed_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, comment="When profile was last analyzed"
    )
    ...
    __table_args__ = (
        ...
        # JSONB indexing for pattern data - temporarily disabled
        # Index(
        #     "idx_personalization_profile_usage_gin",
        #     "usage_patterns",
        #     postgresql_using="gin",
        # ),  # Temporarily disabled
        # Index(
        #     "idx_personalization_profile_features_gin",
        #     "feature_usage",
        #     postgresql_using="gin",
        # ),  # Temporarily disabled
        # Index(
        #     "idx_personalization_profile_behavior_gin",
        #     "behavioral_patterns",
        #     postgresql_using="gin",
        # ),  # Temporarily disabled
        # Index(
        ...
        # ),
        # Index(
        #     "idx_personalization_profile_ui_gin",
        #     "ui_preferences",
        #     postgresql_using="gin",
        # ),
    )
...
class PersonalizationRule(Base):
    ...
    name: Mapped[str] = mapped_column(
        String(200), nullable=False, comment="Human-readable rule name"
    )
    description: Mapped[str] = mapped_column(
        Text, nullable=False, comment="Rule description and purpose"
    )

    # Rule targeting
    target_segments: Mapped[List[str]] = mapped_column(
        JSONB, nullable=False, comment="User segments this rule applies to"
    )
    target_contexts: Mapped[List[str]] = mapped_column(
        JSONB, nullable=False, comment="Contexts where this rule is applied"
    )
    conditions: Mapped[Dict[str, Any]] = mapped_column(
        JSONB, nullable=False, comment="Conditions for rule activation"
    )
    ...
    configuration: Mapped[Dict[str, Any]] = mapped_column(
        JSONB, nullable=False, comment="Personalization configuration parameters"
    )
    ...
    is_active: Mapped[bool] = mapped_column(
        Boolean, default=True, index=True, comment="Whether rule is currently active"
    )
    is_ab_test: Mapped[bool] = mapped_column(
        Boolean, default=False, comment="Whether this is an A/B test rule"
    )
    ab_test_id: Mapped[Optional[str]] = mapped_column(
        String(36), nullable=True, comment="Associated A/B test identifier"
    )

    # Performance tracking
    applications_count: Mapped[int] = mapped_column(
        Integer, default=0, comment="Number of times rule has been applied"
    )
    success_rate: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Success rate of this personalization (0.0-1.0)"
    )
    avg_improvement: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Average improvement metric"
    )
    ...
    starts_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, comment="When rule becomes active"
    )
    expires_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, comment="When rule expires"
    )
    ...
    __table_args__ = (
        # Index(
        #     "idx_personalization_rule_segments_gin",
        #     "target_segments",
        #     postgresql_using="gin",
        # ),  # Temporarily disabled
        # Index(
        #     "idx_personalization_rule_contexts_gin",
        #     "target_contexts",
        #     postgresql_using="gin",
        # ),  # Temporarily disabled
        # Index(
        #     "idx_personalization_rule_conditions_gin",
        #     "conditions",
        #     postgresql_using="gin",
        # ),  # Temporarily disabled
        # Index(
        ...
        # ),  # Temporarily disabled
        Index(
            "idx_personalization_rule_type_active", "personalization_type", "is_active"
        ),
        ...
        Index(
            "idx_personalization_rule_performance", "success_rate", "applications_count"
        ),
        Index("idx_personalization_rule_timing", "starts_at", "expires_at"),
        # JSONB indexing for conditions and configuration - temporarily disabled
        # Index(
        #     "idx_personalization_rule_conditions_gin",
        #     "conditions",
        #     postgresql_using="gin",
        # ),
        # Index(
        #     "idx_personalization_rule_config_gin",
        #     "configuration",
        #     postgresql_using="gin",
        # ),
    )
...
class PersonalizationInteraction(Base):
    ...
    personalization_data: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="Data about the personalization shown"
    )
    user_action: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, comment="Specific user action taken"
    )
    outcome: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True, comment="Outcome: positive, negative, neutral"
    )
    ...
    engagement_score: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Calculated engagement score for this interaction"
    )

    # Session context
    session_id: Mapped[Optional[str]] = mapped_column(
        String(100), index=True, nullable=True, comment="Session identifier"
    )
    device_type: Mapped[Optional[str]] = mapped_column(
        String(20), nullable=True, comment="Device type: mobile, tablet, desktop"
    )
    ...
    rule: Mapped["PersonalizationRule"] = relationship(
        "PersonalizationRule", back_populates="interactions", lazy="selectin"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_personalization_interaction_context_type",
            "context",
            "interaction_type",
        ),
        ...
        Index(
            "idx_personalization_interaction_performance",
            "response_time_ms",
            "engagement_score",
        ),
        # JSONB indexing for personalization data - temporarily disabled
        # Index(
        #     "idx_personalization_interaction_data_gin",
...
class UserPersonalizationRules(Base):
    ...
    is_active: Mapped[bool] = mapped_column(
        Boolean, default=True, comment="Whether this rule is active for this user"
    )
    override_configuration: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB, nullable=True, comment="User-specific rule configuration overrides"
    )
...
class PersonalizationSegmentAnalysis(Base):
    ...
    segment: Mapped[str] = mapped_column(
        String(50), index=True, nullable=False, comment="User segment being analyzed"
    )
    analysis_period: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="Analysis period: daily, weekly, monthly"
    )

    # Segment metrics
    total_users: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="Total users in segment"
    )
    active_users: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="Active users in segment during period"
    )
    avg_session_duration: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Average session duration for segment"
    )
    conversion_rate: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Conversion rate for segment"
    )

    # Personalization effectiveness
    personalization_effectiveness: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Overall personalization effectiveness score"
    )
    top_performing_rules: Mapped[Optional[List[str]]] = mapped_column(
        JSONB, nullable=True, comment="Top performing personalization rules for segment"
    )
    improvement_opportunities: Mapped[Optional[List[str]]] = mapped_column(
        JSONB, nullable=True, comment="Identified improvement opportunities"
    )

    # Timing
    period_start: Mapped[datetime] = mapped_column(
        nullable=False, comment="Start of analysis period"
    )
    period_end: Mapped[datetime] = mapped_column(
        nullable=False, comment="End of analysis period"
    )
    ...
    __table_args__ = (
        Index(
            "idx_personalization_segment_analysis_segment_period",
            "segment",
            "analysis_period",
        ),
        Index(
            "idx_personalization_segment_analysis_period", "period_start", "period_end"
        ),
        # Index(
        #     "idx_personalization_segment_analysis_effectiveness",
        #     "personalization_effectiveness",
        # ),
        Index("idx_personalization_segment_analysis_computed", "computed_at"),
        # JSONB indexing for rules and opportunities - temporarily disabled
        # Index(
        #     "idx_personalization_segment_analysis_rules_gin",
        #     "top_performing_rules",
        #     postgresql_using="gin",
        # ),
        # Index(
        #     "idx_personalization_segment_analysis_opportunities_gin",
