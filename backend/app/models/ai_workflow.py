...
class WorkflowSession(Base):
    ...
    checkpoints: Mapped[List["WorkflowCheckpoint"]] = relationship(
        "WorkflowCheckpoint", back_populates="session", cascade="all, delete-orphan"
    )
    task_batches: Mapped[List["TaskBatch"]] = relationship(
        "TaskBatch", back_populates="session", cascade="all, delete-orphan"
    )
...
class WorkflowCheckpoint(Base):
    ...
    previous_checkpoint_id: Mapped[Optional[str]] = mapped_column(
    ...
    session: Mapped[Optional["WorkflowSession"]] = relationship(
        "WorkflowSession", back_populates="checkpoints"
    )
...
class AgentMessage(Base):
    ...
    )  # pending, delivered, acknowledged, expired
    ...
    # Relationships
    session: Mapped[Optional["WorkflowSession"]] = relationship(
        "WorkflowSession", back_populates="agent_messages"
    )
...
class TaskBatch(Base):
    ...
    # Relationships
    session: Mapped[Optional["WorkflowSession"]] = relationship(
        "WorkflowSession", back_populates="task_batches"
    )
...
class Task(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(50), default="pending"
    )  # pending, scheduled, running, completed, failed, cancelled, rolled_back
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
    ...
    batch: Mapped[Optional["TaskBatch"]] = relationship(
        "TaskBatch", back_populates="tasks"
    )
...
class QualityGateExecution(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(50)
    )  # passed, failed, warning, skipped, running
...
class AgentRegistry(Base):
    ...
    )  # execution, monitoring, quality, custom
    name: Mapped[str] = mapped_column(String(255))
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    ...
    # Capabilities
    supported_task_types: Mapped[Optional[List[str]]] = mapped_column(
        JSONB, nullable=True
    )
...
class WorkflowMetrics(Base):
    ...
    )  # messaging, execution, quality_gates

    # Timing
    recorded_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
    time_window: Mapped[Optional[str]] = mapped_column(
...
# Checkpoint indexes
Index(
    "idx_checkpoints_session_agent",
    WorkflowCheckpoint.session_id,
    WorkflowCheckpoint.agent_id,
)
...
Index(
    "idx_quality_gates_type_status",
    QualityGateExecution.gate_type,
    QualityGateExecution.status,
)
...
# Metrics indexes
Index(
    "idx_metrics_name_component", WorkflowMetrics.metric_name, WorkflowMetrics.component
)
Index(
    "idx_metrics_session_recorded",
    WorkflowMetrics.session_id,
    WorkflowMetrics.recorded_at,
)
