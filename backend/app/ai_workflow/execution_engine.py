...
class TaskExecutionEngine:
    ...
    async def schedule_task(
        self, task: Task, execution_context: Optional[ExecutionContext] = None
    ) -> str:
        ...
        try:
            ...
            logger.error("Failed to schedule task", task_id=task.id, error=str(e))
    ...
    async def execute_batch(
        ...
        try:
            ...
            if batch.execution_strategy == "sequential":
                ...
                raise ValueError(
                    f"Unknown execution strategy: {batch.execution_strategy}"
                )
            ...
            if failed_tasks and execution_context.rollback_enabled:
                ...
                rollback_results = await self._rollback_batch(
                    executed_tasks, execution_context, batch.rollback_strategy
                )
            ...
            logger.info(
                ...
                rolled_back_count=len(
                    [r for r in results if r.status == TaskStatus.ROLLED_BACK]
                ),
            )
            ...
            logger.error("Batch execution failed", batch_id=batch.id, error=str(e))
            ...
            if executed_tasks and execution_context.rollback_enabled:
                try:
                    ...
                    logger.error(
                        "Rollback failed", batch_id=batch.id, error=str(rollback_error)
                    )
    ...
    async def _execute_sequential(
        self, tasks: List[Task], execution_context: ExecutionContext
    ) -> List[TaskResult]:
        ...
        for i, task in enumerate(tasks):
            try:
                ...
                if (
                    ...
                    and execution_context.rollback_enabled
                ):
                    logger.warning(
    ...
    async def _execute_parallel(
        ...
        task_coroutines = [
            self._execute_single_task(task, execution_context) for task in tasks
        ]
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                final_results.append(
                    TaskResult(
                        task_id=tasks[i].id, status=TaskStatus.FAILED, error=str(result)
                    )
                )
            else:
        ...
        while len(completed_tasks) < len(tasks):
            ...
            for task in tasks:
                ...
                dependencies_met = all(
                    dep.task_id in completed_tasks for dep in task.dependencies
                )
    ...
    async def _execute_single_task(
        self, task: Task, execution_context: ExecutionContext
    ) -> TaskResult:
        ...
        try:
            ...
            if task.timeout_seconds:
                result = await asyncio.wait_for(
                    handler(task, execution_context), timeout=task.timeout_seconds
                )
            else:
        ...
        try:
            if rollback_strategy == "reverse_order":
                ...
                for task in reversed(executed_tasks):
                    rollback_result = await self._rollback_single_task(
            ...
            logger.error(
                "Rollback failed", batch_id=execution_context.batch_id, error=str(e)
            )
    ...
    async def _create_execution_checkpoint(
        ...
        try:
            ...
            logger.warning(
                "Failed to create execution checkpoint", batch_id=batch_id, error=str(e)
            )
    ...
    async def validate_completion(self, task_results: List[TaskResult]) -> bool:
        ...
        successful_tasks = [
            result for result in task_results if result.status == TaskStatus.COMPLETED
        ]

        failed_tasks = [
            result for result in task_results if result.status == TaskStatus.FAILED
        ]
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": self._tasks_executed
            / max(self._tasks_executed + self._tasks_failed, 1),
