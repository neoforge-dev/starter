...
class WorkflowStateManager:
    ...
    async def save_checkpoint(
        ...
        try:
            ...
            previous_checkpoint = await self._get_latest_checkpoint(
                session_id, agent_id
            )
    ...
    async def compress_context(self, context_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            temp_context = WorkflowContext(
                session_id="temp", agent_id="temp", context_data=context_data
            )
    ...
    async def _compress_context(self, context: WorkflowContext) -> WorkflowContext:
        ...
        if "conversation_history" in context.context_data:
            ...
            if isinstance(history, list) and len(history) > 50:
                ...
                middle_summary = (
                    f"[Summarized {len(history) - 30} messages from conversation]"
                )
                summary_data["conversation_history"] = (
                    important + [{"summary": middle_summary}] + recent
                )
            else:
        ...
        compression_ratio = (
            original_size - compressed_context.size_bytes
        ) / original_size
        logger.info(
            "Context compressed",
            original_size=original_size,
                    ...
                    async with aiofiles.open(checkpoint_file, "r") as f:
                        ...
                        if checkpoint.agent_id == agent_id:
                            if (
                                ...
                                or checkpoint.created_at > latest_timestamp
                            ):
    ...
    async def _cleanup_old_checkpoints(self, session_id: str, agent_id: str) -> None:
        ...
        try:
            ...
            if len(agent_checkpoints) > self.max_checkpoints_per_session:
                for checkpoint, checkpoint_file in agent_checkpoints[
    ...
    async def list_checkpoints(
        self, session_id: str, agent_id: Optional[str] = None, limit: int = 50
                ...
                if session_dir.is_dir():
                    ...
                    if checkpoints_dir.exists():
                        ...
                        if checkpoint_file.exists():
                            await aiofiles.os.remove(checkpoint_file)
                            logger.info(
                                "Checkpoint deleted", checkpoint_id=checkpoint_id
                            )
                            increment_counter("ai_workflow_checkpoint_deleted")
                            return True

            logger.warning(
                "Checkpoint not found for deletion", checkpoint_id=checkpoint_id
            )
            return False

        except Exception as e:
            logger.error(
                "Failed to delete checkpoint", checkpoint_id=checkpoint_id, error=str(e)
            )
